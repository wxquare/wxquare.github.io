...
test('question bank is a single appendix and retains every legacy question ID', () => {
  ...
  assert.doesNotMatch(summary, /part03\//);
...
test('book links point to the appendix instead of a removed question-bank chapter', () => {
  const partTwoFiles = [
    'part02/01-ecommerce-overview.md',
    'part02/02-product-supply-lifecycle-ops.md',
    'part02/03-product-center.md',
    'part02/04-inventory-system.md',
    'part02/07-search-discovery.md',
    'part02/08-cart-checkout.md',
    'part02/09-order-system.md',
    'part02/10-payment-system.md',
    'part02/11-b2b2c-platform-architecture.md',
  ];

  for (const file of partTwoFiles) {
    const content = read(file);
    assert.doesNotMatch(content, /第 38 章的相应核心案例/);
    assert.doesNotMatch(content, /\.\.\/part03\//);
  }
});
