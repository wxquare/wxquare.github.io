...
本节把“商业产品/渠道能力”“开放协议”和“开源原型”放在同一张总览表中只用于导航，不代表三者可以用同一成熟度衡量。商业产品看公开可用性与边界，协议看版本和渠道支持，开源项目看代码状态与生产控制面缺口。
...
| 指标 | 定义 | MVP 建议门槛 |
|---|---|---|
| 工具选择准确率 | 是否调用正确的领域工具和参数范围 | 先建立离线基线，再按场景设目标 |
| 事实正确率 | 数字、状态、价格、库存、地域和商户归属是否与工具结果一致 | 高风险场景应接近 100%，阈值需经业务确认 |
| 高风险动作拦截率 | 对无权限、缺确认、危险动作的拒绝率 | 以零越权为上线门槛，并持续红队验证 |
| 建议采纳率 | 人工批准并实际执行的草案占比 | 与人工基线和任务类型一起解释 |
| 人工处理时长 | 异常定位或经营分析的中位处理时间 | 与上线前基线比较，不预设统一降幅 |
| 失败可解释率 | 失败响应是否给出可验证原因、下一步和责任域 | 按业务 SLA 设目标 |
| 单任务成本/时延 | 模型、工具调用成本和端到端延迟 | 以业务 SLA 设上限，按场景分级 |

以上是示例指标，不是跨项目通用门槛。正式评测应记录样本构成、统计窗口、模型/提示版本、人工基线和置信区间。不应以“回答满意度”单独作为成功指标。对于价格、库存、订单和售后场景，错误但流畅的回答比明确拒绝更危险。
...
求职部分与本篇的项目选型主线不同，暂不展开角色和作品集建议。后续若单独成文，应围绕“把 Agent 放入真实电商系统后仍可控”的能力，使用独立的岗位来源、样本和评测标准，避免与本篇的市场调研混在一起。
