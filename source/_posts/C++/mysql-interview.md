...
## 存储引擎InnoDB和MyISAM
- MySQL存储引擎MyISAM与InnoDB区别，比较通用的innodb，支持事务
- Innodb引擎提供了对数据库ACID事务的支持。并且还提供了行级锁和外键的约束。MyIASM引擎(原本Mysql的默认引擎)：不提供事务的支持，也不支持行级锁和外键
- innodb数据和索引时集中存储的，插入记录时是按照主键大小有序插入。Myism数据和索引是分开存储的，插入数据时是按照插入顺序保存的
- innodb是聚簇索引，myisam是非聚簇索引。聚簇索引的叶子节点就是数据节点，而非聚簇索引的叶子节点仍然是索引节点，只不过是指向对应数据块的指针
...
## 索引index
- 什么是索引，对索引的理解，索引通常用于优化查询性能
- 在什么场景下用索引，where，order by,join on
- 索引的类型，主键索引、唯一索引和普通索引
- 单列索引和联合索引
- 覆盖索引
- 怎么创建和删除索引
- 索引的数据结果，B+树、B树、hash索引，各自的优缺点及用途。hash索引能快速查询，但不支持范围查询
- 联合索引的存储结构
- 为什么innodb表必须有主键，并且推荐使用整型的自增主键？因为在innodb表中，数据和主键索引用B+Tree来组织的，没有主键innodb会生成唯一列，类似于rowid
- 聚簇索引和非聚簇索引。innodb表是聚簇索引，叶子节点中包含了完整的数据记录；MYISAM是非聚簇索引，索引和数据是分开存储的
- 什么是最左前缀原则？什么是最左匹配原则
