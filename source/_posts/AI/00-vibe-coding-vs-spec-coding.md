...
而另一种方式是：先花时间写清规范、约束和验收标准，再让 AI 按步骤实现并验证。这就是 **Spec Coding**（规范驱动编程）。它有助于减少需求遗漏和反复沟通，但结果仍取决于任务、模型和规范质量，必须经过测试与评审。
...
|------|-------------|-------------|
| 需求表达 | 边做边补充，容易遗漏约束 | 先明确目标、边界和验收标准 |
| 反馈方式 | 依赖多轮试错 | 每个任务都有明确的验证步骤 |
| 适用场景 | 探索、原型和快速验证 | 需要可复现、可评审的生产任务 |
| 主要风险 | 返工和技术债务可能累积 | 规范维护成本增加，仍需人工判断 |
...
规范把需求、约束和验证方式显式化，帮助团队更早发现遗漏；它不能保证 AI 一次生成正确代码。
...
|------|-------------|-------------|------|
| **开发时间** | 视任务而定 | 视任务而定 | 不能脱离任务比较 |
| **返工次数** | 取决于反馈质量 | 取决于验证结果 | 规范有助于提前暴露问题 |
| **代码行数** | 300 行 | 280 行 | 持平 |
| **测试覆盖** | 由项目决定 | 由项目验收标准决定 | 不能用固定比例概括 |
| **Bug 数** | 需通过测试与评审确认 | 需通过测试与评审确认 | 不能预先保证为零 |
| **可维护性** | 中 | 高 | 显著提升 |
| **文档完整性** | 无 | 规范即文档 | 质的飞跃 |
| **团队协作** | 困难 | 容易 | 显著提升 |
