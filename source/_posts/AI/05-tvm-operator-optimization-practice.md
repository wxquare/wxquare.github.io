...
| 原文章 | 关键内容 | 新文章位置 |
|---|---|---|
| 初识 TVM | 选择 TVM 的背景、安装、TensorFlow 对比、fallback 警告 | 第 1、20 节 |
| Relay 和图优化 | Hello Relay、卷积单元、Pass、优化前后 IR | 第 3、4、24、29、30 节 |
| GEMM 优化 | 浮点峰值、NumPy、手工优化阶梯、TVM 阶段数据 | 第 5、6、21、22、23 节 |
| 代码生成 | `tvm.build`、`relay.build`、IRModule、PrimFunc、Module、Codegen | 第 7、25 节 |
| 模型量化 | 量化动机、跨平台部署、RFC、INT8 测试表 | 第 8、26、31 节 |
| 学习资料 | 安装、向量加法、TensorFlow 导入、量化资料 | 第 26 节和参考资料 |

这张表不是文章正文的装饰，而是重构验收记录。每次后续更新都应对照它，确保合并不是只保留新写的概念，而是没有遗漏原博客的一手代码和测试数据。
...
本次合并遵循以下比例：正文说明保留原段落和原意的 85%～95%，代码和命令保留 95% 以上，公式和实验数据保留 100%，原文结论保留 90% 以上。重复背景只保留信息最完整的一版，原文章之间的互相引用删除，改成本文的章节关系。任何新增的代码如果没有在原文中出现，都必须标注为“当前版本建议写法”或“实验封装示意”，不称作历史实测代码。

最终文章的可信度来自三条链路：归档文件保存原始事实，新文章通过覆盖表说明事实位于哪个章节，参考资料为术语和算法提供公开依据。后续如果重新运行实验，应新增“当前复测”列，而不是覆盖历史数据。

## 35. 复核结论

逐篇对照归档文件后，原文的主线、代码入口、关键 Pass、GEMM 阶段表、量化性能表、历史环境和已知问题都已经进入新文章。新增段落只承担连接和解释作用，没有用新的假设替换原始实验。对于当前 API 不兼容的片段，文章同时保留旧版代码语境和当前版本迁移边界，读者可以先理解原始实验，再决定是否在新环境中重跑。正文中的每一组数字都应能够追溯到旧文章的表格或输出，新增参考资料只用于解释背景、校准术语和说明当前实现，不改变原作者当时观察到的结果。这样既保持博客的个人实践痕迹，也让新的读者能够沿着代码、命令、数据和结论逐步复核整个优化过程。
