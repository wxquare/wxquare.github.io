...
## 1.环境
anaconda+sublime+Python3的交互式环境
...
2. python是动态类型语言，可以把任意数据类型赋值给变量，同一个变量可以反复赋值，而且可以是不同类型的变量。  
    a = 123 # a是整数  
    print(a)  
    a = 'ABC' # a变为字符串  
    print(a)  
...
## 9.如何导入模块？
1. import module,然后使用module.function调用函数
2. from module import function，直接调用函数


## 10.在python程序之前指定python版本是个好习惯
```
...
5. 关键字参数和默认参数
6. 不定参数，可变参数（*），不定关键字参数（**）
7. 作用域（函数局部变量和如何使用全局变量global）
8. 函数式编程（map、filter、reduce）
