...
> **电商系统设计（十三）**（转化链路专题；总索引见{% post_link system-design/20-ecommerce-overview （一）全景概览与领域划分 %}）
...
- **支付流程实现**：见{% post_link system-design/27-ecommerce-payment-system 支付系统 %}，本篇只写"提交订单成功 → 跳转支付"的衔接点。
- **订单状态机全展开**：见{% post_link system-design/26-ecommerce-order-system 订单系统 %}，本篇只写"结算页创建订单"的调用契约。
...
**系列扩展阅读（不在本文展开）**：履约与物流（订单拆单后的仓库路由与物流追踪）、商家结算与对账（平台抽佣与结算周期）；若你正在补齐支付链路，可接续阅读{% post_link system-design/27-ecommerce-payment-system 支付系统 %}与{% post_link system-design/26-ecommerce-order-system 订单系统 %}。

---

## 参考资料

1. 本系列：{% post_link system-design/26-ecommerce-order-system 订单系统 %} · {% post_link system-design/24-ecommerce-pricing-engine 计价引擎 %} · {% post_link system-design/22-ecommerce-inventory 库存系统 %} · {% post_link system-design/23-ecommerce-marketing-system 营销系统 %} · {% post_link system-design/21-ecommerce-product-center 商品中心 %} · {% post_link system-design/27-ecommerce-payment-system 支付系统 %}。
