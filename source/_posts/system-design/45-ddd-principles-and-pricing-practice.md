...
DDD 处理的不是代码行数，而是业务知识在多人、多系统和长期变化中不断丢失的问题。[Eric Evans 的 Domain-Driven Design](https://www.domainlanguage.com/ddd/) 把模型、语言与实现放在同一条反馈回路里；[Fowler 对 Domain-Driven Design 的概述](https://martinfowler.com/bliki/DomainDrivenDesign.html) 也强调复杂领域与模型之间的联系。所谓“驱动”，指需求讨论、代码命名、边界划分和测试断言都接受领域模型约束，而不是建模结束后再把图放进文档。
...
DDD 不强制采用特定分布式架构。[Richardson 的 CQRS pattern](https://microservices.io/patterns/data/cqrs.html) 适用于读写模型确有不同需求的场景；[Fowler 对 Event Sourcing 的说明](https://martinfowler.com/eaaDev/EventSourcing.html) 讨论用事件重建状态的代价与能力；[Microsoft 的 Microservices architecture 指南](https://learn.microsoft.com/en-us/azure/architecture/guide/architecture-styles/microservices) 则描述独立部署边界。CQRS、事件溯源和微服务都不是 DDD 的必选项：模块化单体、普通状态存储和同一读写模型完全可以承载成熟的领域模型。
...
场景是：运营说订单金额指优惠前，客服说优惠后，财务说含税应收。这里真正需要决定的不是选择哪个框架，而是先识别三个语境是否属于不同上下文，再为各自口径命名。模型必须明确每个金额的构成、币种、时间点和所有者；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：术语表中的正反例、接口字段定义、三方共同认可的订单样例。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现一个 totalAmount 在各系统被不同公式写入，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。

这个案例还要回答三项运行问题：谁对错误结果负责，旧结果如何回放，策略变化怎样兼容历史数据。答案分别落入所有者与告警、版本化证据、迁移与弃用规则。只有设计、代码、测试和运行手册表达同一决定，模型才算完成，而不是在类图评审结束时完成。


#### 协作信号：一次促销要五个团队同时上线

场景是：新增券类型需要商品、营销、计价、订单和支付一起改枚举。这里真正需要决定的不是选择哪个框架，而是找出变化真正属于哪个模型，并通过稳定契约传播结果。模型必须明确上游新增规则不会迫使无关下游理解内部类型；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：版本化候选优惠契约、消费方测试、独立发布记录。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现共享 SDK 每次加常量都要求全链路升级，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：总额正确但没人能还原哪些规则命中和如何分摊。这里真正需要决定的不是选择哪个框架，而是把价格解释视为计价结果的一部分而非日志补丁。模型必须明确总额由可追踪明细唯一推导，明细关联规则与输入版本；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：PriceBreakdown、回放工具、事故样例的确定性结果。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现只能从多服务日志猜测计算顺序，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：单测必须启动数据库、缓存和多个远程服务。这里真正需要决定的不是选择哪个框架，而是把纯业务决策与数据取得、事务和协议翻译分离。模型必须明确给定固定领域输入即可重复得到同一裁决；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：快速表格测试、端口替身、少量真实契约测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现每个边界样例都依赖不稳定的共享环境，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：计价组合频繁变化，而规则后台大多是简单维护。这里真正需要决定的不是选择哪个框架，而是把深入建模集中在核心决策，不把全系统统一成重型模板。模型必须明确核心域获得清晰模型，支撑域保持足够简单；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：变更频率、事故成本、差异化价值与维护周期。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现用代码生成器制造大量没有行为的领域对象，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
领域模型需要一组分工清楚的构件，而不是一套类名清单。[Fowler 对 Bounded Context 的说明](https://martinfowler.com/bliki/BoundedContext.html) 指出模型只在明确边界内成立；[Fowler 对 Domain Model 的说明](https://martinfowler.com/eaaDev/DomainModel.html) 则解释了数据与行为共同表达业务规则的意义。下面每个概念都按定义、计价映射、边界和失败信号展开。
...
在计价域中，Money、Currency、RulePeriod、Quantity 和 Percentage 都应在边界处拒绝非法值，并提供具有业务语义的运算。这个映射不是为了给旧代码换一组名词，而是为了回答“谁作决定、哪些状态必须一起成立、变化被限制在哪里”。[ISO 4217 currency codes](https://www.iso.org/iso-4217-currency-codes.html) 提供币种代码语义，但精度、舍入和现金最小单位仍需业务政策明确。
...
聚合是一组需要共同守护不变量的对象，聚合根是外部修改它们的唯一入口。[Vernon 的聚合讨论与实现资料](https://vaughnvernon.com/) 强调小聚合和按业务规则划边界。
...
仓储让应用以领域语言取得和保存聚合。[Fowler 的企业应用架构模式目录](https://martinfowler.com/eaaCatalog.html) 给出 Repository 与 Data Mapper 等模式的关系。
...
领域事件记录模型中已经发生且其他参与者关心的事实。[Fowler 对事件驱动架构的分类](https://martinfowler.com/articles/201701-event-driven.html) 提醒团队区分通知、状态转移和事件携带状态。
...
场景是：两份内容相同的报价分别发给两个购物车。这里真正需要决定的不是选择哪个框架，而是报价按身份区分，金额按取值比较。模型必须明确quoteId 表示生命周期，Money 相等不依赖存储地址；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：实体相等性测试、值对象相等性测试和恢复样例。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现所有对象都用数据库主键或所有对象都按字段比较，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：接受报价时需要确认版本和有效期，但不需要修改商品。这里真正需要决定的不是选择哪个框架，而是只把同事务不变量放进报价聚合。模型必须明确Accepted 状态、版本与成交明细一次保存后共同成立；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：并发 Accept 测试、乐观锁冲突和过期拒绝。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现为了少写接口把商品、用户和活动全部加载进聚合，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：多个优惠候选需要互斥、择优与封顶。这里真正需要决定的不是选择哪个框架，而是把无自然实体归属的组合政策放进无状态服务。模型必须明确相同候选和基数产生确定的 RuleDecision；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：组合表格、平局规则、预算封顶和顺序无关性质测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现服务一边计算一边查询数据库并发送消息，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：客服按手机号检索近三月报价，订单按 quoteId 接受一个报价。这里真正需要决定的不是选择哪个框架，而是命令路径用聚合仓储，检索路径用专用读模型。模型必须明确写模型不因报表字段而膨胀，查询不绕过写入规则；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：仓储接口、查询 DTO、权限和数据新鲜度说明。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现一个通用 Repository 暴露任意过滤与任意更新，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
战略设计的产物不是服务列表，而是业务能力、语言边界、数据所有权和协作关系。[Microsoft 的 DDD 领域分析指南](https://learn.microsoft.com/en-us/azure/architecture/microservices/model/domain-analysis) 提供从业务能力识别模型边界的工程视角。对计价来说，目标是避免“价格”成为所有上下文都能改写的共享概念。

### 3.1 从事件时间线发现决策边界

工作坊先让业务参与者按时间写出事实，再补充触发事实的命令、作出决定的角色、外部政策与热点问题。[Fowler 的 Event Storming 条目](https://martinfowler.com/bliki/EventStorming.html) 与 [Brandolini 的 EventStorming 官方站点](https://www.eventstorming.com/) 都把领域事件作为探索业务流程的入口。计价链路可从“商品已选定、候选优惠已取得、报价已计算、报价已接受、订单已创建、退款已完成”开始，而不是先画已有服务。

事件之间的规则密集处往往是核心决策边界，语言突然变化处往往是上下文边界。例如“规则命中”属于营销与计价协作，“报价接受”把可变建议转成订单承诺，“支付扣款”则进入支付上下文。边界不是由一次会议永久确定；事故、组织调整和新业务都会提供修正证据。

### 3.2 五个上下文的计价地图

**商品上下文**拥有商品身份、规格与可售属性，并向计价提供版本化商品事实；它不决定会员折扣。**价格上下文**拥有基础价、渠道价和价格表版本；它不解释营销资格。**营销上下文**拥有活动、券、预算与资格，输出候选调整及约束。**用户上下文**拥有会员等级、分群与权益事实，但不计算订单成交价。**订单上下文**接受报价、保存成交快照并管理履约与售后承诺。
...
场景是：展示类目调整后品类券突然失效。这里真正需要决定的不是选择哪个框架，而是区分导航分类与计价分类，并通过反腐层翻译。模型必须明确计价规则读取版本化计价分类，不依赖前台目录结构；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：分类契约、版本、迁移映射和历史报价回放。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现计价直接连接商品分类表，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：报价时是金卡，下单时等级被后台修正。这里真正需要决定的不是选择哪个框架，而是定义用户事实的新鲜度和报价承诺强度。模型必须明确报价记录会员事实版本，接受时按政策校验或锁定；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：等级版本、有效时间、重价提示和客服解释。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现每一步都读取最新等级却没有差异处理，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：候选优惠有效但预算在并发请求中耗尽。这里真正需要决定的不是选择哪个框架，而是区分资格判断、预算预占和最终核销的所有者。模型必须明确计价不直接修改营销预算，凭证有期限且幂等；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：预占标识、过期释放、确认事件和补偿记录。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现Calculate 方法跨库扣预算，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：活动下线后发生部分退款。这里真正需要决定的不是选择哪个框架，而是订单保存成交证据，营销继续拥有规则定义。模型必须明确退款不依赖当前规则仍在线，也不篡改历史报价；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：快照模式版本、分摊、规则引用和退款政策。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现退款时重新调用当前计价引擎，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：营销要增加新的互斥作用域。这里真正需要决定的不是选择哪个框架，而是由上下游关系决定兼容、试运行和弃用窗口。模型必须明确旧消费者在窗口内仍能解释已有字段；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：契约版本、示例、消费方测试和责任人。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现共享群消息代替正式变更协议，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
战术设计把战略边界变成代码可以守护的约束。[Vernon 的 Implementing Domain-Driven Design](https://vaughnvernon.com/) 对聚合、领域事件和上下文集成给出系统化实践；可运行示例可参考 [Vaughn Vernon 维护的 IDDD Samples](https://github.com/VaughnVernon/IDDD_Samples)。下面不重复定义清单，而是沿一份报价从创建到接受的生命周期组织模型。
...
func ParseMoney(text, currency string, scale int) (Money, error) {
    if scale < 0 || scale > 4 {
        return Money{}, fmt.Errorf("unsupported scale: %d", scale)
    }
    parts := strings.Split(text, ".")
    if len(parts) > 2 || parts[0] == "" {
        return Money{}, errors.New("invalid decimal amount")
    }
    if strings.HasPrefix(parts[0], "-") {
        return Money{}, errors.New("negative amount is not allowed")
    }
    fraction := ""
    if len(parts) == 2 {
        fraction = parts[1]
    ...
    if len(fraction) > scale {
        return Money{}, errors.New("amount requires explicit rounding")
    }
    fraction += strings.Repeat("0", scale-len(fraction))
    whole, err := strconv.ParseInt(parts[0], 10, 64)
    if err != nil || strings.HasPrefix(fraction, "-") {
        return Money{}, errors.New("invalid decimal amount")
    }
    frac := int64(0)
    if fraction != "" {
        frac, err = strconv.ParseInt(fraction, 10, 64)
        if err != nil { return Money{}, errors.New("invalid decimal amount") }
    }
    factor := int64(1)
    for i := 0; i < scale; i++ { factor *= 10 }
    if whole > (math.MaxInt64-frac)/factor {
        return Money{}, errors.New("amount overflow")
    }
    return Money{minor: whole*factor + frac, currency: currency}, nil
}
```

需要四舍五入的输入应调用名字明确的 ParseMoneyRounded，并传入 HalfEven、HalfUp 等策略；不要让 ParseMoney 猜测。负数是否允许也由用途决定：商品单价可以拒绝负数，调整额则用独立 Discount 类型表达方向。
...
```go
func (q *PriceQuote) Accept(now time.Time, cartVersion int64) error {
    if q.status != QuoteCalculated { return ErrInvalidQuoteState }
    if !now.Before(q.expiresAt) { return ErrQuoteExpired }
    if cartVersion != q.cartVersion { return ErrCartVersionChanged }
    q.status = QuoteAccepted
    q.events = append(q.events, QuoteAccepted{QuoteID: q.id, At: now})
    return nil
}
```
...
聚合记录事件，应用层在业务数据保存成功后安排发布。数据库事务无法与消息代理天然原子提交时，可采用 [Transactional Outbox 模式](https://microservices.io/patterns/data/transactional-outbox.html)：业务记录与 outbox 同事务写入，后台发布器至少一次投递，消费者按 eventId 幂等。Outbox 解决丢消息窗口，不提供全局顺序或恰好一次语义。

领域事件转换为集成事件时应删除内部字段、补充模式版本和关联标识。[CloudEvents 1.0](https://cloudevents.io/) 可统一事件信封，[AsyncAPI 3.0 文档](https://www.asyncapi.com/docs) 可描述通道、消息和操作。契约兼容规则与消费方测试比“所有事件都放同一个 JSON”更能保护演进。
...
场景是：客户端发送 12.345 CNY。这里真正需要决定的不是选择哪个框架，而是拒绝多余精度或调用显式舍入接口，不能静默截断。模型必须明确每个 Money 的最小单位整数和币种始终合法；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：非法输入、溢出、负数和舍入模式测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现float64 乘一百后直接转 int64，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：过期报价与用户提交同时发生。这里真正需要决定的不是选择哪个框架，而是由聚合在同一版本上判断状态与时间。模型必须明确过期报价不能 Accepted，已接受报价不能重写明细；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：边界时刻、并发版本、重复 Accept 和恢复测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现控制器直接更新 status 字段，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：数据库提交成功后消息代理短暂不可用。这里真正需要决定的不是选择哪个框架，而是在同事务保存 outbox，再异步重试发布。模型必须明确每个已提交事实最终可发布，消费者处理重复投递；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：故障注入、积压指标、eventId 去重和重放测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现先发消息再保存或提交后只尝试发送一次，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：报价用例需要三个依赖和一次事务。这里真正需要决定的不是选择哪个框架，而是应用层协调，领域层裁决，适配器翻译。模型必须明确业务公式不出现在应用服务，领域对象不启动事务；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：端口接口、用例测试和领域纯单元测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现一个 Service 同时拼 SQL、算折扣和发 Kafka，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：运营需要按活动统计优惠成本。这里真正需要决定的不是选择哪个框架，而是从事件或变更流构建分析投影，不扩张报价聚合。模型必须明确投影可重建且标注新鲜度，不能反向修改领域状态；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：投影版本、重建命令、水位和对账测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现为报表给聚合增加大量无行为字段，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
func (s *QuoteApplicationService) Calculate(
    ctx context.Context,
    cmd CalculateQuoteCommand,
) (QuoteView, error) {
    if saved, ok := s.idempotency.Find(ctx, cmd.RequestID); ok {
        return saved, nil
    }

    input, err := NewPricingInput(cmd)
    if err != nil { return QuoteView{}, MapDomainError(err) }

    prices, err := s.priceCatalog.Load(ctx, input.ProductRefs(), cmd.EvaluatedAt)
    if err != nil { return QuoteView{}, DependencyError("price-catalog", err) }
    promotions, err := s.promotions.Candidates(ctx, input.PromotionQuery())
    if err != nil { return QuoteView{}, DependencyError("promotion", err) }

    quote, err := s.calculator.Calculate(input, prices, promotions)
    if err != nil { return QuoteView{}, MapDomainError(err) }

    if err := s.tx.Within(ctx, func(tx Tx) error {
        if err := s.quotes.Save(tx, quote); err != nil { return err }
        return s.outbox.Append(tx, IntegrationEventsFrom(quote.Events()))
    }); err != nil {
        return QuoteView{}, err
    }

    view := ToQuoteView(quote)
    s.idempotency.Remember(ctx, cmd.RequestID, view)
    return view, nil
}
```

这段编排里有四个值得保留的决定。第一，EvaluatedAt 来自请求而不是领域内部读取系统时钟，使历史回放和边界测试可重复。第二，价格和促销输入携带各自版本，报价可以证明依据。第三，报价与 outbox 同事务保存，避免保存成功但事件丢失。第四，幂等存储关联业务请求与结果；相同键若携带不同购物车版本，应返回冲突而不是复用旧结果。
...
HTTP 适配器可以采用 [RFC 9457 Problem Details](https://www.rfc-editor.org/rfc/rfc9457.html)，以稳定 type 区分问题类别，用扩展字段携带 quoteId、ruleId、currentVersion 和 retryable。API 契约用固定的 [OpenAPI 3.1 规范](https://spec.openapis.org/oas/v3.1.0.html) 描述，不把 latest 链接写成某个固定版本。错误文案可以本地化，错误类型与字段语义必须保持稳定。
...
场景是：同一购物车在不同渠道和时刻得到不同结果。这里真正需要决定的不是选择哪个框架，而是让渠道、地区、用户事实、评估时间、币种与版本全部显式。模型必须明确相同完整输入得到相同决定，缺少关键事实直接拒绝；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：请求摘要、输入版本串、幂等键和回放样例。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现模型内部随时读取系统时钟或全局用户上下文，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：五十元满减要分到三个订单行并支持部分退款。这里真正需要决定的不是选择哪个框架，而是选择稳定分摊基数和确定尾差归属。模型必须明确行分摊之和等于五十元且每行不超可分摊金额；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：比例、平局、最小单位、全退与多次部分退测试。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现展示时临时分摊，退款时又按新规则重算，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：自动推荐优惠超时但基础价可用。这里真正需要决定的不是选择哪个框架，而是返回明确 Degraded 状态并由下单政策决定能否接受。模型必须明确降级不会伪装完整承诺，缺失权益可被监控和解释；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：缺失依赖、风险等级、用户提示和转化影响指标。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现基础设施适配器直接返回零优惠，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：新旧引擎最终总额相同但行分摊不同。这里真正需要决定的不是选择哪个框架，而是比较总额、明细、命中原因和版本，而不只比较一个数字。模型必须明确灰度订单可追踪引擎来源并完整走退款对账；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：差异分类、阈值、自动回退和失败样本库。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现只看成功率便宣布迁移完成，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
[Hohpe 与 Woolf 的 Enterprise Integration Patterns](https://www.enterpriseintegrationpatterns.com/) 可用于明确消息通道、路由、端点和相关标识；[Kleppmann 的 Designing Data-Intensive Applications](https://dataintensive.net/) 有助于理解复制、流与一致性的工程取舍；[Fowler 的 Patterns of Distributed Systems](https://martinfowler.com/articles/patterns-of-distributed-systems/) 则提供可复用的分布式协调模式。模式只描述机制，计价团队仍需定义业务上允许的延迟和补偿。

### 6.5 CQRS 与服务拆分的采用门槛

写模型需要守护报价不变量，客服和运营查询却可能需要按用户、规则、渠道和时间聚合。只有当直接查询写库已明显妨碍模型或性能时，再引入独立读模型。[Microsoft 的 CQRS pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) 提醒读写分离会带来同步延迟和双模型治理；团队必须准备投影重建、版本迁移和陈旧数据提示。

微服务也应在模块边界稳定后采用。[Richardson 的 Microservices Patterns 入口](https://microservices.io/) 总结 Saga 等跨服务模式，但网络边界会新增超时、重试、观测和发布协调。若计价与营销仍共享大量表、术语和同步发布，先做模块化单体更诚实。部署边界可以晚于模型边界，反过来却很难用网络修复混乱模型。
...
场景是：会员标签允许分钟级延迟，基础价必须实时确认。这里真正需要决定的不是选择哪个框架，而是分别定义新鲜度与失败政策，而不是统一 RPC 或消息。模型必须明确每类事实的陈旧上限和降级后果可被调用者理解；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：SLA、缓存版本、积压水位和重价差异。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现因技术偏好把所有依赖改成同一种通信，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：模块计算负载高但规则与营销仍频繁共同修改。这里真正需要决定的不是选择哪个框架，而是先独立扩展计算适配器或模块，再验证领域边界。模型必须明确部署变化不迫使模型共享数据库或泄漏内部类型；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：性能数据、独立发布率、调用失败预算和团队所有权。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现按代码行数拆服务后仍同步发布，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：运营复杂查询拖慢报价写入。这里真正需要决定的不是选择哪个框架，而是比较索引、只读副本和独立投影的成本后再选择。模型必须明确写模型仍守护规则，读模型声明陈旧并可重建；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：查询负载、投影延迟、重建时间和故障演练。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现为简单列表复制完整读写模型，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：QuoteAccepted 要增加承担方分摊。这里真正需要决定的不是选择哪个框架，而是新增可选字段、版本化语义并给消费者迁移窗口。模型必须明确旧消费者仍能处理，新消费者能识别字段来源；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：AsyncAPI 示例、消费方测试、弃用日期和使用清单。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现直接改字段含义却保持原版本，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：报价延迟正常但优惠成本突然翻倍。这里真正需要决定的不是选择哪个框架，而是把技术指标与规则版本、命中率和成本指标关联。模型必须明确报警能指向业务所有者并保留决策证据；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：规则维度仪表盘、DecisionTrace 和异常样本。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现只有 CPU、P99 和统一错误率，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
幂等、超时、重试和降级不是统一中间件参数，它们会改变用户承诺。[AWS Builders’ Library](https://aws.amazon.com/builders-library/) 对超时、重试和分布式系统实践提供了生产视角。计价团队应分别定义基础价、优惠、税和事件发布的失败政策，避免基础设施层用同一个默认值处理所有依赖。
...
场景是：旧引擎难维护但仍承载全部订单。这里真正需要决定的不是选择哪个框架，而是按语言、模型、影子、灰度和下线分阶段验证。模型必须明确每阶段可回退且只有一个事实源；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：阶段验收、差异报告、流量标记和回退演练。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现新系统一次性复制旧系统所有隐含行为，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：团队把聚合数量和事件数量当成改造成果。这里真正需要决定的不是选择哪个框架，而是用交付周期、事故解释和边界独立性评价模型。模型必须明确每个模式对应明确业务问题和退出条件；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：基线指标、变更记录、事故恢复时间和团队反馈。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现为了覆盖模板创建无行为类和无消费者事件，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：术语表、接口、代码和客服话术各自更新。这里真正需要决定的不是选择哪个框架，而是让变更评审同时更新模型资产和验收示例。模型必须明确同一术语在边界内的定义可从多入口一致验证；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：链接到测试的示例、契约检查和定期抽查。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现模型只存在于过期幻灯片，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
...
场景是：支撑域规则趋于稳定且维护投入超过收益。这里真正需要决定的不是选择哪个框架，而是保留清晰边界但降低战术复杂度。模型必须明确简化不破坏数据所有权、历史兼容和关键不变量；边界外只通过契约获得必要事实，不能把别处的数据表或内部对象当成自己的状态。

可验收证据包括：变更频率、维护成本、替代方案和回退计划。测试至少覆盖正常路径、边界时刻、重复请求和依赖失败，并断言业务结果而非内部调用顺序。若实现出现把 DDD 当成不可逆的组织身份，说明知识仍散落在流程或基础设施中，应回到语言、所有权和不变量重新划分。
