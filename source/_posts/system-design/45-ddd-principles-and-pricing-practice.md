...
date: 2026-09-23
description: 以电商计价为案例，解释统一语言、限界上下文、领域模型与价格快照如何支撑复杂业务的演进和治理。
categories:
  - 系统设计基础
tags:
...
// 示例白名单；生产环境应由受版本控制的 ISO 4217 元数据生成。
var currencyMinorUnits = map[string]int{
    "CNY": 2, "USD": 2, "JPY": 0, "KWD": 3,
}

func ParseMoney(text, currency string) (Money, error) {
    currency = strings.ToUpper(strings.TrimSpace(currency))
    scale, ok := currencyMinorUnits[currency]
    if !ok { return Money{}, fmt.Errorf("unsupported currency: %q", currency) }
...
func (s *QuoteApplicationService) Calculate(
    ...
    fingerprint := input.Fingerprint() // 包含 CartVersion 与规范化输入摘要

    if saved, ok, err := s.idempotency.Find(ctx, cmd.RequestID); err != nil {
        return QuoteView{}, err
    } else if ok {
        if saved.Fingerprint != fingerprint { return QuoteView{}, ErrIdempotencyConflict }
    ...
    if err := s.tx.Within(ctx, func(tx Tx) error {
        // Claim 依赖 request_id 唯一约束；并发请求只能有一个创建成功。
        claim, err := s.idempotency.Claim(tx, cmd.RequestID, fingerprint)
        if err != nil { return err }
        if claim.Existing != nil {
            if claim.Existing.Fingerprint != fingerprint { return ErrIdempotencyConflict }
            view = claim.Existing.View
            return nil
        }
        if err := s.quotes.Save(tx, quote); err != nil { return err }
        if err := s.outbox.Append(tx, IntegrationEventsFrom(quote.Events())); err != nil { return err }
        return s.idempotency.Complete(tx, cmd.RequestID, fingerprint, view)
    }); err != nil {
        return QuoteView{}, err
    }
    return view, nil
