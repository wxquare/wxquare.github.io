...
**参考**：{% post_link system-design/41-acc-clean-arch-ddd-cqrs 架构与整洁代码（一） %} §1（分层与依赖规则）。
...
**参考**：{% post_link system-design/41-acc-clean-arch-ddd-cqrs 41-架构方法论 %} §2.1（限界上下文）。
...
**参考**：{% post_link system-design/41-acc-clean-arch-ddd-cqrs 41-架构方法论 %} §2.5（聚合）。
...
**参考**：{% post_link system-design/41-acc-clean-arch-ddd-cqrs 41-架构方法论 %} §3（CQRS 与读写分离）。
...
**参考**：{% post_link system-design/41-acc-clean-arch-ddd-cqrs 41-架构方法论 %} §5.3（反模式与 YAGNI）。
...
**参考**：{% post_link system-design/42-acc-clean-code 42-acc-clean-code %} 中与「意图命名」相关的章节（配合 §4 Pipeline 组织用例）。
...
**参考**：{% post_link system-design/41-acc-clean-arch-ddd-cqrs 41-架构方法论 %} §2.7（领域事件与集成）。
...
| 场景特征 | 推荐模式 | 参考 |
|----------|----------|------|
| 多步骤顺序流程 | Pipeline（管道） | {% post_link system-design/42-acc-clean-code 42-acc-clean-code §4 %} |
| 同一接口多种实现 | 策略模式 | {% post_link system-design/42-acc-clean-code 42-acc-clean-code §6.1 %} |
| 频繁变化的业务规则 | 规则引擎 / 规则表驱动 | {% post_link system-design/42-acc-clean-code 42-acc-clean-code §7 %} |
| 跨聚合协作 | 领域事件 + Outbox | {% post_link system-design/41-acc-clean-arch-ddd-cqrs 41-架构方法论 §2.7 %} |
...
**反例**：一个 `Handle` 内顺序完成：鉴权、解析、校验、调用下游、重试、日志、指标、错误映射——应拆为 **小函数** 或 **Pipeline 阶段**（参见 {% post_link system-design/42-acc-clean-code 42-acc-clean-code §4 %}）。
