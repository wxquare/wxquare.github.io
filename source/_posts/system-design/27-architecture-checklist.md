...
| [复杂业务中的 Clean Code 实践指南](/system-design/15-clean-code/)（15） | **怎么写**：函数、Pipeline、策略、规则引擎等战术 |
| [Clean Architecture、DDD 与 CQRS：三位一体的架构方法论](/system-design/26-clean-architecture-ddd-cqrs/)（26） | **怎么设计**：分层、BC、聚合、CQRS、事件与反模式 |
...
**参考**：[Clean Architecture、DDD 与 CQRS：三位一体的架构方法论](/system-design/26-clean-architecture-ddd-cqrs/) §1（分层与依赖规则）。
...
**参考**：[26-架构方法论](/system-design/26-clean-architecture-ddd-cqrs/) §2.1（限界上下文）。
...
**参考**：[26-架构方法论](/system-design/26-clean-architecture-ddd-cqrs/) §2.5（聚合）。
...
**参考**：[26-架构方法论](/system-design/26-clean-architecture-ddd-cqrs/) §3（CQRS 与读写分离）。
...
**参考**：[26-架构方法论](/system-design/26-clean-architecture-ddd-cqrs/) §5.3（反模式与 YAGNI）。
...
**参考**：[15-clean-code](/system-design/15-clean-code/) 中与「意图命名」相关的章节（配合 §4 Pipeline 组织用例）。
...
**参考**：[26-架构方法论](/system-design/26-clean-architecture-ddd-cqrs/) §2.7（领域事件与集成）。
...
| 场景特征 | 推荐模式 | 参考 |
|----------|----------|------|
| 多步骤顺序流程 | Pipeline（管道） | [15-clean-code §4](/system-design/15-clean-code/) |
| 同一接口多种实现 | 策略模式 | [15-clean-code §6.1](/system-design/15-clean-code/) |
| 频繁变化的业务规则 | 规则引擎 / 规则表驱动 | [15-clean-code §7](/system-design/15-clean-code/) |
| 跨聚合协作 | 领域事件 + Outbox | [26-架构方法论 §2.7](/system-design/26-clean-architecture-ddd-cqrs/) |
...
**反例**：一个 `Handle` 内顺序完成：鉴权、解析、校验、调用下游、重试、日志、指标、错误映射——应拆为 **小函数** 或 **Pipeline 阶段**（参见 [15-clean-code §4](/system-design/15-clean-code/)）。
...
- [复杂业务中的 Clean Code 实践指南](/system-design/15-clean-code/)
- [Clean Architecture、DDD 与 CQRS：三位一体的架构方法论](/system-design/26-clean-architecture-ddd-cqrs/)
...
系统化的 Code Review 不是挑剔，而是**把重构前移到成本最低的阶段**。按 **架构 → 设计 → 代码 → 上线前** 四段清单推进，并与 [15-clean-code](/system-design/15-clean-code/)、[26-架构方法论](/system-design/26-clean-architecture-ddd-cqrs/) 交叉引用，团队可以在一致的语言下讨论分层、边界与实现细节。建议把本文的「附录快速参考」嵌入 MR 模板，并在复盘时根据失效案例**增补你们自己的第 21 条**——最好的 Checklist 永远是活文档。
