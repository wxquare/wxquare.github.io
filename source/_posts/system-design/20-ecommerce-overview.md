...
### 1.1 业务架构（BA）：核心业务能力全景

**视角说明**：从业务能力的角度，展示电商平台的核心业务域和功能模块。

```mermaid
graph TB
    subgraph 电商业务架构
        subgraph C端业务域[🛒 C端业务域：消费者购物]
            direction TB
            C1[商品发现<br/>·商品搜索<br/>·商品详情<br/>·推荐系统]
            C2[购物决策<br/>·价格计算<br/>·优惠券领取<br/>·库存查询]
            C3[交易履约<br/>·订单创建<br/>·支付<br/>·订单跟踪]
            
            C1 --> C2 --> C3
        end
        
        subgraph B端业务域[🏪 B端业务域：商家供给与运营]
            direction TB
            B1[商品供给<br/>·商品上架<br/>·审核发布<br/>·批量导入]
            B2[商品管理<br/>·商品信息编辑<br/>·价格调整<br/>·库存补货]
            B3[营销运营<br/>·优惠券配置<br/>·活动管理<br/>·首页配置]
            
            B1 --> B2
            B2 --> B3
        end
        
        subgraph 核心数据域[📊 核心数据域：支撑服务]
            direction LR
            D1[商品中心<br/>·SPU/SKU<br/>·类目属性<br/>·商品快照]
            D2[库存中心<br/>·库存管理<br/>·预订扣减<br/>·对账]
            D3[计价中心<br/>·价格计算<br/>·费用配置<br/>·营销规则]
        end
        
        subgraph 资金域[💰 资金域：交易与结算]
            direction LR
            F1[支付服务<br/>·支付<br/>·退款<br/>·对账]
            F2[清结算<br/>·商家分账<br/>·平台抽佣<br/>·财务报表]
        end
    end
    ...
    %% 样式
    classDef cDomain fill:#e3f2fd,stroke:#2196f3,stroke-width:3px
    classDef bDomain fill:#f3e5f5,stroke:#9c27b0,stroke-width:3px
    classDef dataDomain fill:#e8f5e9,stroke:#4caf50,stroke-width:3px
    classDef moneyDomain fill:#fff3e0,stroke:#ff9800,stroke-width:3px
    
    class C1,C2,C3 cDomain
    class B1,B2,B3 bDomain
    class D1,D2,D3 dataDomain
    class F1,F2 moneyDomain
```

**业务能力层级**：

| 业务域 | 一级能力 | 二级能力（文章覆盖） | 面向用户 |
|-------|---------|---------------------|---------|
| **C端业务域** | 商品发现 | 商品搜索、商品详情、推荐（未详述） | C端消费者 |
| | 购物决策 | 价格计算、优惠券、库存查询 | C端消费者 |
| | 交易履约 | 订单创建、支付、订单跟踪 | C端消费者 |
| **B端业务域** | 商品供给 | 商品上架、审核发布、批量导入 | 商家/供应商 |
| | 商品管理 | 商品编辑、价格调整、库存补货 | 运营人员 |
| | 营销运营 | 优惠券配置、活动管理、首页配置 | 运营人员 |
| **核心数据域** | 商品中心 | SPU/SKU模型、类目属性、快照 | 所有业务域 |
| | 库存中心 | 库存管理、预订扣减、对账 | 所有业务域 |
| | 计价中心 | 价格计算、费用配置、营销规则 | 所有业务域 |
| **资金域** | 支付服务 | 支付、退款、对账 | C端/B端 |
| | 清结算 | 商家分账、平台抽佣、财务报表 | B端/财务 |
