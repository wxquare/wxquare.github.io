...
**视角说明**：采用 DDD（领域驱动设计）的**核心域、支撑域、通用域**三层结构，展示完整的电商业务能力。本章节帮助你理解**业务价值**与**投资优先级**的关系。
...
<details>
<summary>📊 Mermaid 版本图表（点击展开，便于在线预览）</summary>
...
#### 1.1.2 DDD 三层域定义与投资策略

| 域类型 | 定义 | 业务价值 | 投资策略 | 典型例子 | 本系列覆盖 |
|-------|------|---------|---------|---------|----------|
| **🎯 核心域**<br/>(Core Domain) | 平台的核心竞争力，<br/>业务差异化的关键 | **最高**<br/>决定平台成败 | **重点投入，必须自研**<br/>顶尖团队 + 持续优化 | 订单编排、支付清结算、<br/>购物车试算 | ✅ 订单、支付、购物车 |
| **📦 支撑域**<br/>(Supporting Domain) | 支撑核心业务的必要能力，<br/>有一定复杂度但非差异化 | **中等**<br/>必须有但不是卖点 | **适度投入，可定制开发**<br/>参考行业方案 + 适配 | 商品管理、库存扣减、<br/>营销规则、上架流程 | ✅ 商品、库存、计价、<br/>营销、上架、运营、生命周期 |
| **🔧 通用域**<br/>(Generic Domain) | 通用基础能力，<br/>行业标准化程度高 | **低**<br/>无差异化 | **最小投入，采购或开源**<br/>优先使用成熟方案 | 用户登录（SSO）、<br/>短信推送、风控平台 | ✅ 搜索（重要性升级）<br/>❌ 其他通用域（采购） |

**关键洞察**：
- **核心域**：占系统 20%，但带来 80% 的业务价值 → **必须自研，持续优化**
- **支撑域**：占系统 50%，是核心域的地基 → **可定制，确保稳定性**
- **通用域**：占系统 30%，标准化程度高 → **采购或开源，降低成本**

---

#### 1.1.3 完整领域能力图谱（13 篇文章覆盖）
...
| **评价域** | 商品评价、DSR 评分 | ❌ 未详述 | 评价系统通用 | 参考淘宝/京东 |
| **物流域** | 配送履约、物流跟踪 | ❌ 未详述 | 对接三方物流 | 顺丰/菜鸟 API |
| **风控域** | 反欺诈、风险识别 | ❌ 未详述 | 风控平台成熟 | 同盾/数美风控 |
...
| 判断维度 | 核心域 | 支撑域 | 通用域 |
|---------|-------|-------|-------|
| **差异化程度** | 平台独特能力 | 有一定特色但可参考 | 行业标准化 |
| **失败影响** | 致命（如支付错误） | 严重但可容忍（如库存超卖） | 影响体验但非致命 |
| **团队配置** | 最强团队 + 持续优化 | 中等团队 + 稳定迭代 | 外包或采购 |
| **投资回报** | 高（核心竞争力） | 中（必须但不是卖点） | 低（成本中心） |
| **技术选型** | 自研 + 深度优化 | 定制 + 参考开源 | 采购 + 对接 |

**常见误区（面试陷阱）**：

❌ **误区 1**：把所有自己做的都当核心域  
✅ **正确**：搜索虽然重要但技术方案成熟（ES + 召回），属于通用域（重要性升级）

❌ **误区 2**：通用域不重要所以随便做  
✅ **正确**：通用域也要稳定可靠，只是**采购/开源优于自研**

❌ **误区 3**：核心域永远不变  
✅ **正确**：业务发展会导致域升级（如搜索从通用域升级为支撑域）
...
> 3. **投资回报**：投入的人力能带来多大价值？
> 
> 例如订单系统，差异化高（拆单策略各家不同）、失败影响大（订单错误致命）、ROI 高（直接影响 GMV），所以是核心域。而搜索虽然重要，但 ES + 召回方案已成熟，属于通用域（重要性升级）。"
...
| | 生命周期域 | 上架/同步/编辑边界 | （十一）生命周期管理 | 商品流转协调 |
...
3. 校验通过后：
  - 生成订单；
  - 保存商品快照（包括商品名、图片、下单时价格、规格、活动信息）；
4. 即使商品后续修改或下架，订单中的快照数据不受影响。
```
