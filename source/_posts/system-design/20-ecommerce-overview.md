...
graph TB
    ...
    subgraph Layer4[🔧 第四层：运营管理层]
        LIST[商品上架系统<br/>流程编排<br/>审核策略<br/>状态机]
        OPS[B端运营系统<br/>日常维护<br/>批量管理<br/>数据看板]
    end
    ...
    %% B端供给阶段：商品上架（橙色虚线）
    MERCHANT -->|① 商品上架<br/>商品信息+初始价格+初始库存| LIST
    LIST -.->|② 创建商品| PC
    LIST -.->|③ 初始化库存| INV
    LIST -.->|④ 配置基础价格| PRICE
...
| 系统 | 依赖的系统 | 提供给哪些系统 | 核心职责 | B端操作 |
|-----|----------|--------------|---------|---------|
| **商品中心** | - | 库存、计价、订单、商品上架、B端运营 | 商品数据存储与服务 | 被上架系统创建、被运营系统编辑 |
| **库存系统** | 商品中心 | 订单、B端运营 | 库存管理与扣减 | 被上架系统初始化、被运营系统修改 |
| **营销系统** | - | 计价引擎、B端运营 | 优惠规则与补贴 | 被运营系统配置 |
| **计价引擎** | 商品中心、营销系统 | 订单、B端运营 | 价格计算与快照 | 被上架系统配置、被运营系统修改 |
| **订单系统** | 商品中心、库存、计价 | 支付 | 订单创建与状态管理 | - |
| **支付系统** | 订单 | - | 支付、退款、清结算 | - |
| **商品上架** | 商品中心、库存、计价 | - | 流程编排、状态机、审核 | ① 供给阶段：创建商品+初始化 |
| **B端运营** | 商品中心、库存、营销、计价 | - | 日常维护、批量管理 | ⑤ 管理阶段：编辑商品+修改配置 |

**关键数据流**：

- **C端流程**（实线）：用户 → 浏览商品 → 下单 → 支付
- **B端供给**（虚线①-④）：商家 → 商品上架系统 → 创建商品/库存/价格
- **B端管理**（虚线⑤-⑪）：运营 → B端运营系统 → 编辑商品/库存/价格/营销
...
系统中存在三条独立的数据流，分别服务于不同的业务场景：

| 数据流 | 起点 | 路径 | 终点 | 特点 |
|-------|------|------|------|------|
| **C端购物流** | C端用户 | 商品中心 → 库存 → 计价 → 订单 → 支付 | 交易完成 | 实线箭头，查询为主 |
| **B端供给流** | 商家/供应商 | 商品上架系统 → 商品中心/库存/计价 | 商品上线 | 虚线箭头①-④，创建操作 |
| **B端管理流** | 运营人员 | B端运营系统 → 商品中心/库存/计价/营销 | 配置更新 | 虚线箭头⑤-⑪，修改操作 |

**关键洞察**：

- **商品上架系统** ≠ 商品中心，它是**流程编排器**，负责审核、状态机、工作流
- **B端运营系统** ≠ 数据系统，它是**管理界面**，负责批量操作、配置管理
- **商品中心/库存/计价** 是**数据服务**，被上架系统创建、被运营系统修改
