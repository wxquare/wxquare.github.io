...
**导航**：[书籍主页](./index.html) | [完整目录](./TOC.html) | [上一章](./chapter4.html) | [下一章](./chapter6.html)

---

# 第5章 电商系统全景图

> 本章定位：在读完第一部分的方法论之后，进入第二部分「电商核心系统设计」之前，用一张**可落地的全景图**把业务能力、应用系统、数据资产与技术基础设施对齐到同一坐标系。后续第 7 章至第 15 章将沿本章划定的边界逐域深入；第 6 章则在全景之上讨论跨系统集成与一致性模式。
...
本书将**订单、支付、购物车与结算**作为交易链路主轴（第 13 章至第 15 章），并在第 6 章从一致性模式上把它们串成可复用的集成语言。
...
**优点**：实现直观、调试路径短。  
**风险**：级联故障、线程占用、超时风暴；需配合**超时、重试、熔断、舱壁**与清晰的**错误契约**（第 6 章 6.6 节）。
...
**风险**：重复消息、乱序、滞后；需**幂等消费、版本号、可补偿流程**（第 1 章 Outbox、第 6 章事件驱动）。
...
|------|------|----------|------------|----------|
| 1 | 商品中心 | SPU/SKU、类目属性、商品快照与主数据质量 | 库存扣减、营销计算、支付 | [第 7 章](./chapter7.html) |
| 2 | 库存系统 | 可售量、预占/确认/释放、对账与供应商同步 | 价格计算、订单状态机 | [第 8 章](./chapter8.html) |
| 3 | 营销系统 | 券/活动/补贴、圈品、预算与防刷 | 订单持久化、支付渠道 | [第 9 章](./chapter9.html) |
| 4 | 计价系统 | 多场景试算、费用、价格快照与降级 | 营销资金账、库存数量 | [第 11 章](./chapter11.html) |
| 5 | 搜索与导购 | Query、召回、排序、Hydrate 编排 | 不作为订单或支付事实来源 | [第 12 章](./chapter12.html) |
| 6 | 购物车 | 行项目暂存、合并、批量操作 | 不持有支付契约 | [第 13 章](./chapter13.html) |
| 7 | 结算编排 | 结算页 Saga、预占协调、拆单预览 | 不替代订单合同存储 | [第 13 章](./chapter13.html) |
| 8 | 订单系统 | 合同、状态机、拆单、履约协调入口 | 渠道密钥、资金划拨 | [第 14 章](./chapter14.html) |
| 9 | 支付系统 | 支付单、渠道路由、回调、对账与退款 | 商品主数据、库存数量 | [第 15 章](./chapter15.html) |
| 10 | 商品上架 | 上架审核、发布流程、供给侧状态机 | 不复制商品中心全量模型职责 | [第 10 章](./chapter10.html) |
| 11 | 供给与运营管理 | 批量任务、配置工具、权限与审计 | 不绕过商品中心直接写「影子库」 | [第 10 章](./chapter10.html) |
| 12 | 生命周期与供给治理 | 同步/编辑/下架边界、跨系统编排约束 | 不实现全量搜索召回 | [第 10 章](./chapter10.html) |
...
**与第 6 章的衔接**：第 6 章将把 5.3 的交互模式上升为 **Saga、幂等、对账、事件驱动** 等一致性语言，并把 CAP 折中讲透。建议在读第 6 章前，先用 5.4 的时序图遮住文字，尝试**口述一遍**每条箭头上的失败与补偿，检验是否已建立全景肌肉记忆。
