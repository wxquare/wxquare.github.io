...
`source/library/` 是本仓库公开第三方参考资料的规范入口和迁移目标。`source/booklist/` 是已阻塞的只读遗留目录，逐条权威来源与元数据完成前保持原位；`source/pdf/` 是只读遗留目录，已批准的第一方演示资料等待 Task 4 迁入 `source/presentations/`。两个遗留目录均不得新增文件。资料公开可读并不自动代表本仓库可以重新托管；不能确认再分发边界时，只记录原始链接。
