...
**Goal:** 编写一篇面向面试候选人的电商支付系统文章（29-ecommerce-payment-system.md），覆盖核心流程、状态机、分布式事务等高频考点

**Architecture:** 单文件 Markdown 文章，按章节组织内容（7 章），包含 Mermaid 图表、代码示例、表格等

**Tech Stack:** Markdown, Mermaid (图表), Hexo (静态博客生成器)
...
**Create:**
- `source/_posts/system-design/29-ecommerce-payment-system.md` - 支付系统文章主文件

**Reference (已存在):**
- `source/_posts/system-design/26-ecommerce-order-system.md` - 参考订单系统的写作风格
- `source/_posts/system-design/28-ecommerce-marketing-system.md` - 参考营销系统的结构
- `docs/superpowers/specs/2026-04-09-ecommerce-payment-system-design.md` - 设计文档

---

## Task 1: 创建文章文件和 Front Matter

**Files:**
- Create: `source/_posts/system-design/29-ecommerce-payment-system.md`

- [ ] **Step 1: 创建文章文件**

在 `source/_posts/system-design/` 目录下创建 `29-ecommerce-payment-system.md`
...
运行：`ls -la source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：文件存在

- [ ] **Step 5: 提交**

```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: create payment system article with front matter"
```

---

## Task 2: 编写第 1 章 - 业务背景与挑战

**Files:**
- Modify: `source/_posts/system-design/29-ecommerce-payment-system.md`
...
运行：`cat source/_posts/system-design/29-ecommerce-payment-system.md | grep "## 一、业务背景与挑战"`  
预期：章节标题存在
...
```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: add chapter 1 - business context and challenges"
```

---

## Task 3: 编写第 2 章 - 整体架构设计
...
运行：`grep -A 50 "```mermaid" source/_posts/system-design/29-ecommerce-payment-system.md | head -60`  
预期：Mermaid 图表语法正确
...
```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: add chapter 2 - architecture design with mermaid diagram"
```

---

## Task 4: 编写第 3 章 - 核心业务流程
...
运行：`grep -c "```go" source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：至少 3 个 Go 代码块
...
```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: add chapter 3 - core business flows (payment & refund)"
```

---

## Task 5: 编写第 4 章 - 状态机设计
...
- [ ] **Step 2: 验证 Mermaid 状态图**

运行：`grep -c "stateDiagram-v2" source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：至少 2 个状态图
...
```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: add chapter 4 - state machine design"
```

---

## Task 6: 编写第 5 章 - 高频考点深入
...
运行：`grep -c "### 5\." source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：至少 3 个三级标题（5.1, 5.2, 5.3）

- [ ] **Step 3: 提交**

```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: add chapter 5 - key topics (distributed tx, idempotency, consistency)"
```
...
运行：`grep -c "graph LR" source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：至少 2 个流程图
...
```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: add chapter 6 - extended modules (settlement, reconciliation, risk)"
```

---

## Task 8: 编写第 7 章 - 面试问答锦囊
...
运行：`grep -c "^#### Q" source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：8 个问题
...
```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: add chapter 7 - interview Q&A cheatsheet"
```
...
**Files:**
- Verify: `source/_posts/system-design/29-ecommerce-payment-system.md`
...
运行：`ls -la public/2026/04/09/29-ecommerce-payment-system/index.html`  
预期：文件存在

- [ ] **Step 4: 本地预览（可选）**

运行：`npm run server`  
访问：`http://localhost:4000/2026/04/09/29-ecommerce-payment-system/`  
预期：文章正常显示
...
**Files:**
- Commit: `source/_posts/system-design/29-ecommerce-payment-system.md`

- [ ] **Step 1: 检查文章完整性**

运行：`wc -l source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：文件行数 > 1000

- [ ] **Step 2: 检查图表数量**

运行：`grep -c '```mermaid' source/_posts/system-design/29-ecommerce-payment-system.md`  
预期：至少 8 个 Mermaid 图表

- [ ] **Step 3: 最终提交**

```bash
git add source/_posts/system-design/29-ecommerce-payment-system.md
git commit -m "feat: complete payment system article

- 17,000 words covering all core topics
- 10 diagrams (architecture, sequence, state machine, flow)
- 7 chapters with interview Q&A cheatsheet
- Target audience: interview candidates
...
- Extended modules (settlement, reconciliation, risk)
- Interview Q&A with 8 high-frequency questions"
```
...
**预计总时长**：60-90 分钟（分 10 个 task，每个 6-9 分钟）
