...
- 参考资料统一使用标准数字编号：正文写作 `[[1]](#ref-1)`、`[[2]](#ref-2)`，页面显示为 `[1]`、`[2]`；文末对应条目必须包含唯一的 `<a id="ref-1"></a>`、`<a id="ref-2"></a>` 锚点；编号按文章内首次出现顺序维护，禁止新增长期使用 `[R01]` 或不可跳转的裸 `[1]` 引用。
- 文章内部引用继续使用 `{% post_link %}`；外部参考资料必须放在“参考资料”或“参考资源”章节，并与正文引用编号一一对应。
