...
- **章节篇幅**：每章正文包含 `10,000–20,000` 个中文汉字。统计范围包括正文段落、表格和图表中的中文文字；英文、数字、标点、代码、链接、参考资料和附录不计入汉字数量，也不得通过重复观点、堆砌引用、无效代码或重复表格凑数。
...
- 章节中文汉字数量默认控制在 `10,000–20,000` 个；如果用户明确指定更窄的范围，以用户指定范围为准，并按内容完整性控制，不以扩展重复观点作为达标手段。
