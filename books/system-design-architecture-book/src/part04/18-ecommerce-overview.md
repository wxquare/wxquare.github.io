...
**答辩提示**：业务架构图的表述模板已统一收录到[第 35 章](../part05/33-ecommerce-architecture-interview.md)。
...
|------|------|----------|------------|----------|
| 1 | 商品中心 | SPU/SKU、类目属性、商品快照与主数据质量 | 库存扣减、营销计算、支付 | [第 21 章](./19-product-center.md) |
| 2 | 库存系统 | 可售量、预占/确认/释放、对账与供应商同步 | 价格计算、订单状态机 | [第 22 章](./20-inventory-system.md) |
| 3 | 营销系统 | 券/活动/补贴、圈品、预算与防刷 | 订单持久化、支付渠道 | [第 23 章](./21-marketing-system.md) |
| 4 | 计价系统 | 多场景试算、费用、价格快照与降级 | 营销资金账、库存数量 | [第 25 章](./23-pricing-system.md) |
| 5 | 搜索与导购 | Query、召回、排序、Hydrate 编排 | 不作为订单或支付事实来源 | [第 26 章](./24-search-discovery.md) |
| 6 | 购物车 | 行项目暂存、合并、批量操作 | 不持有支付契约 | [第 27 章](./25-cart-checkout.md) |
| 7 | 结算编排 | 结算页 Saga、预占协调、拆单预览 | 不替代订单合同存储 | [第 27 章](./25-cart-checkout.md) |
| 8 | 订单系统 | 合同、状态机、拆单、履约协调入口 | 渠道密钥、资金划拨 | [第 28 章](./26-order-system.md) |
| 9 | 支付系统 | 支付单、渠道路由、回调、对账与退款 | 商品主数据、库存数量 | [第 29 章](./27-payment-system.md) |
| 10 | 商品上架 | 上架审核、发布流程、供给侧状态机 | 不复制商品中心全量模型职责 | [第 24 章](./22-product-supply-ops.md) |
| 11 | 供给与运营管理 | 批量任务、配置工具、权限与审计 | 不绕过商品中心直接写「影子库」 | [第 24 章](./22-product-supply-ops.md) |
| 12 | 生命周期与供给治理 | 同步/编辑/下架边界、跨系统编排约束 | 不实现全量搜索召回 | [第 24 章](./22-product-supply-ops.md) |
