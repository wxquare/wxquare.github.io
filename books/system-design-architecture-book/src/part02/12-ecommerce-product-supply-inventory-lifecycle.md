...
这 4 个阶段不是要求所有供应商都做成高并发切片框架，而是提供一个统一的执行心智。对 Agoda 这类慢源 Pull，`Phase 0` 仍然可以只产出一个或少量串行分片，`Phase 1` 保持单 Worker 慢拉，后面的 `Transformer / Publisher` 再通过 MQ 并发消峰。

这里直接定调：

> 对 Agoda 这类慢源，正确答案不是“Fetcher 阶段盲目并发拉”，而是“Sharder 控边界，Fetcher 串行慢拉，Publisher 之后再用 MQ 蓄水和下游并发消峰”。
...
在进入具体执行模型之前，可以先把这一节的核心设计收敛成几张总表。这样读者会先抓住骨架，再去看后面的分层展开、时序图和表结构。
...
这个拆分的最大价值在于：把“拉数据的不稳定”“翻译的复杂性”“发布的治理”彻底解耦。对于 Agoda 这类慢源、大体量、强治理场景，这种分层方式的长期维护性会明显高于“大 Consumer 泥球”。

##### 3.6.2.2 数据库表设计亮点

| 表 | 角色定位 | 为什么重要 |
| --- | --- | --- |
| `supplier_sync_task` | 作战计划模板 + 总控台 | 定义同步任务如何触发、采用哪种分片策略，并承接主任务级管理与观测 |
| `supplier_sync_shard_task` | 分片级执行控制台 | 记录每个 shard 的执行上下文、checkpoint、失败定位与局部恢复 |
| `supplier_sync_batch` | 批次级租约 + 进度总览 | 记录本次批次由谁在跑、跑到哪、是否还能被接管 |
| `supplier_sync_state_ledger` | 对象级状态机中枢 | 承接并发防踩脚、对账、补偿、幽灵消息裁剪四大主权 |
| `supplier_product_mapping_tab` | 映射权威 | 维护外部资源和平台资源之间的稳定映射关系 |

这里最需要强调的是：`supplier_sync_state_ledger` 不是可选项，而是工业级流水线的指挥中心。没有它，100w+ 对象同步很容易退化成黑盒。

##### 3.6.2.3 关键设计点整理

| 模块 | 当前设计 | 补充建议 |
| --- | --- | --- |
| `Sharder` | 支持 `Full / Delta / Cursor` 可插拔策略 | 增加 `shard_strategy_version`，保证策略升级后老批次仍可按原规则恢复 |
| `Fetcher` | Agoda 采用“串行 + 页级 checkpoint + 页级批量物化” | RAW snapshot 旁边补一个 `raw_hash`，辅助判断内容是否真的变化 |
| `Transformer` | 将外部语义映射、标准化从 Fetcher 剥离 | 生成 `normalized_hash + field_version_map`，为 Publisher 提供更细粒度的字段主导权判断依据 |
| `Publisher` | CAS + 实时版本反查 + 写前保护 | 增加乐观字段主导权机制，例如价格、房态、状态可偏供应商主导，其它字段保留平台人工主导权 |
| 全量 List 自动下线 | 批次收尾 + 时间戳打卡 + 候选集 + 比例熔断 | 建议增加 `offline_candidate_batch` 临时表，并配置最小存活比例阈值，防止大面积误下线 |

这张整理表不是要替代后面的小节，而是先把供应商同步设计的几个“高价值判断”显式列出来，方便读者快速建立全局心智。

##### 3.6.2.4 决策点：纯串行 + Checkpoint，还是分阶段 + 分片流水线
...
对 Agoda 这种慢源、QPS 严格受限、上游接口不稳定的场景，更合理的落地方式其实是：
...
- 既保住了 Agoda 场景下最关键的稳定性和 checkpoint 恢复能力；
- 又不会把整个系统锁死在“单 Worker 黑盒长任务”的演进路径上。

##### 3.6.2.5 `supplier_sync_state_ledger` 为什么是关键中枢
...
##### 3.6.2.6 Phase 0: Sharder 的设计点

| 设计点 | 说明 |
| --- | --- |
| `Sharder` 决定同步边界，不负责业务裁决 | 它只回答“这次同步要处理谁、切成多大颗粒度”，不负责酒店字段映射和商品最终发布裁决。 |
| 分片策略必须可插拔 | 全量同步和增量同步的切法完全不同，至少要支持 `FullSharder / DeltaSharder / CursorSharder` 三类策略。 |
| 分片策略要带版本号 | `shard_strategy_version` 建议显式落在主任务或批次上下文里。后续当分片规则升级时，老批次仍能按当时策略做恢复和复盘。 |
| shard 要能独立重试与补偿 | 一个 shard 失败时，应该允许局部重跑，而不是让整个 100w 任务回到起点。 |
...
| shard 元数据必须持久化 | `shard_id / sync_mode / page_range / hotel_id_list / checkpoint / status` 不能只在内存里，否则无法做断点续跑和精确补偿。 |
| 慢源允许退化成单 shard 串行模式 | Agoda 这类接口不稳定、QPS 严格受限的场景，即便有 `Sharder`，最终也可能只生成一个长串行 shard，这不是退步，而是工程上更稳。 |

##### 3.6.2.7 Phase 1: Fetcher 的设计点
...
| RAW 侧建议保留 `raw_hash` | 这不是最终跳过判断依据，但能明显提升重复页识别、问题排查和观测分析能力。 |
| `Fetcher` 只忠实搬运，不做最终裁决 | 上游拉取层负责忠实搬运与状态推进，不基于本地镜像去做最终跳过判断。否则最容易因为失败状态、排队延迟或运营改动造成静默丢单。 |
| Push / Pull 介质分离 | Push 更偏接口受理 + MQ 削峰，Pull 更偏单 Worker 串行拉取 + checkpoint；两者执行模型不同，但治理主线复用。 |

##### 3.6.2.8 Phase 2: Transformer 的设计点

| 设计点 | 说明 |
| --- | --- |
| 映射与标准化应从 Fetcher 剥离 | 把外部接口抖动处理和内部语义转换拆开，能明显降低执行层复杂度。 |
| Transformer 产物要可回放 | 标准化后的对象也要落 `normalized_ref`，这样后续排障、复算、规则调整时不必重新打供应商接口。 |
| 标准化结果建议带 `normalized_hash` | 它适合做观测、调试和局部重复短路，但不应替代商品主域最终幂等裁决。 |
| 字段级主导权判断应有辅助信息 | `field_version_map` 或等价结构能帮助 Publisher 做更细粒度的字段主导权判断，而不是整对象盲覆盖。 |
| 风险识别可以前移，但不做最终发布裁决 | 这一层可以标记异常价格、缺失字段、高风险类目，但不能越权决定是否覆盖正式商品。 |
| Supplier Mapping 独立持久化 | 外部资源和平台资源的映射不能只靠内存或日志，要有权威 `supplier_product_mapping_tab`。 |

##### 3.6.2.9 Phase 3: Publisher 的设计点
...
| CAS 强占消费权 | Publisher Consumer 不能拿到消息就直接处理，而要先把对象状态从 `INIT` 原子推进到 `PROCESSING`，这样才能避免 MQ 重投或多机并发时重复处理同一对象。 |
...
| 字段主导权要显式建模 | 供应商不一定对所有字段都有覆盖权。价格、房态、状态这类高动态字段可以偏供应商主导；标题、卖点、运营标签往往仍应以平台人工编辑为准。 |
...
##### 3.6.2.10 Shard 失败与错误处理任务

对于 Agoda 这类长任务，最忌讳的一件事就是把异常恢复逻辑塞回主同步链路，最后把 `Shard -> Fetch -> Transform -> Publish` 重新写成一个满是分支的大黑盒。更稳的做法是：
...
为了避免把架构分层埋进时序细节里，这里先单独定义供应商同步的四阶段执行模型，再在后面用时序图展开关键链路。这样读者会先知道职责边界，再去看具体执行顺序。

##### 3.6.3.1 Phase 0: Sharder（任务分片器）
...
```json
{
  ...
  "shard_id": "agoda-delta-023",
  "supplier_id": "agoda",
  "sync_mode": "DELTA",
  "hotel_id_list": ["AGD_HOTEL_78231", "AGD_HOTEL_78232"],
  "fetch_cursor": null,
  "page_range": null,
  "checkpoint": null,
  "priority": 50
...
对 Agoda 这类慢源，虽然可以在概念上保留 `Sharder`，但很多时候真正落地会退化成：
...
- `supplier_sync_task` 是 Agoda 酒店同步这类任务的“作战计划模板 + 总控台”。
...
{
  "id": 2001,
  "task_code": "AGODA_HOTEL_DELTA_SYNC",
  "supplier_id": 88,
  "sync_mode": "PULL",
  "data_scope": "HOTEL",
...
[
  {
    ...
    "shard_payload": {
      "hotel_ids": ["AGD_HOTEL_78231", "AGD_HOTEL_78232", "AGD_HOTEL_78233"],
  ...
  },
  {
    ...
    "shard_type": "HOTEL_ID_LIST",
    "shard_payload": {
      "hotel_ids": ["AGD_HOTEL_78731", "AGD_HOTEL_78732", "AGD_HOTEL_78733"],
      "source_change_cursor": "chg_20260604015900_002"
    },
    "worker_id": "fetcher-04",
    "lease_until": "2026-06-04 02:05:00",
    "checkpoint": "hotel_id=AGD_HOTEL_78732",
    "status": "RUNNING",
    "retry_count": 1,
...
```json
{
  ...
  "task_code": "AGODA_HOTEL_FULL_SYNC",
  "shard_no": 42,
...
##### 3.6.3.3 Phase 2: Transformer（数据转换器）

`Transformer` 的输入是 RAW snapshot，输出是内部标准酒店模型。它主要做 4 类工作：

1. 外部字段映射，例如 Agoda 的酒店、房型、价格日历字段映射到平台内部模型。
...
##### 3.6.3.4 Phase 3: Publisher（数据发布器）
...
#### 3.6.6 Agoda 酒店同步样例数据

下面给一组简化样例，帮助把这些表和 Agoda 酒店同步场景串起来。这里默认同步对象是 `AGD_HOTEL_78231`，同步模式是串行 Pull，目标是把供应商侧的酒店基础信息、房型信息和价格/库存差异同步到平台治理链路。

| 对象 | 样例数据 | 说明 |
| --- | --- | --- |
| `supplier_sync_task` | `task_code=AGODA_HOTEL_PULL_DAILY`<br>`sync_mode=PULL`<br>`data_scope=HOTEL`<br>`sharder_type=DELTA_SHARDER`<br>`shard_strategy_version=v3`<br>`concurrency_policy=SINGLE_WORKER` | 业务级同步定义，表达“每天串行拉取 Agoda 酒店数据，并采用可插拔的分片策略” |
...
| `supplier_sync_shard_task` | `batch_id=sb_20260604_0001`<br>`shard_task_id=sst_20260604_0023`<br>`shard_no=23`<br>`shard_type=HOTEL_ID_RANGE`<br>`shard_payload=[78231...78730]`<br>`status=RUNNING`<br>`checkpoint=hotel_id=78510` | 对四阶段框架里的 `ShardContext` 做持久化，表达“本批次切出的一个具体 shard task” |
| `supplier_sync_error_task` | `error_task_id=set_20260604_0007`<br>`source_shard_task_id=sst_20260604_0023`<br>`recovery_type=RETRY_PUBLISH`<br>`target_stage=PUBLISH`<br>`status=INIT`<br>`start_checkpoint=object_key=AGD_HOTEL_78510` | 当 shard 失败后，单独用错误处理任务承接恢复与治理，而不是污染主同步链路 |
...
| `HBase / Object Store` | `raw_ref=oss://agoda-sync/raw/78231.json`<br>`raw_hash=sha256:abc...`<br>`normalized_ref=oss://agoda-sync/norm/78231.json`<br>`normalized_hash=sha256:def...` | 原始报文和标准化对象都沉淀到 HBase / 对象存储，供回放、复算和审计使用 |
...
2. 商品中心不经过普通供给编辑流，直接把正式商品状态推进到 `BANNED` 或 `OFFLINE`。
