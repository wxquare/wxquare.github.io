...
> 本章定位：在读完第一部分的方法论之后，进入第四部分「电商系统设计实战」之前，用一张**可落地的全景图**把业务能力、应用系统、数据资产与技术基础设施对齐到同一坐标系。后续第 21 章至第 32 章将沿本章划定的边界逐域深入；第 6 章已经从方法论层面讨论了跨系统集成与一致性模式。
...
**答辩提示**：业务架构图的表述模板已统一收录到[第 35 章](../part05/03-ecommerce-architecture-interview.md)。
...
| 10 | 商品上架 | 上架审核、发布流程、供给侧状态机 | 不复制商品中心全量模型职责 | [第 24 章](./05-product-supply-ops.md) |
| 11 | 供给与运营管理 | 批量任务、配置工具、权限与审计 | 不绕过商品中心直接写「影子库」 | [第 24 章](./05-product-supply-ops.md) |
| 12 | 生命周期与供给治理 | 同步/编辑/下架边界、跨系统编排约束 | 不实现全量搜索召回 | [第 24 章](./05-product-supply-ops.md) |
