...
库存语义混淆的答辩提示已统一收录到[第 36 章](../part03/04-product-inventory-marketing-pricing-interview.md)。
...
> **与营销库存的关系**：商品库存回答“有没有货”，营销库存回答“活动名额 / 补贴预算够不够”。秒杀等场景往往需要双扣减（商品 + 营销），本章在 22.7 节说明集成边界；营销细节见第 24 章。

### 22.2.6 库存创建：从商品发布到库存实例
...
**跨库存类型一致性（商品 + 营销）**：秒杀场景下商品预占成功但营销失败时，必须回滚商品预占。回滚失败不要把系统留在「半占用」状态：应记录缺陷单并阻塞该 `order_id` 的继续支付，直到补偿成功或人工判定。该话题与第 6 章 Saga、第 23 章营销库存紧密相关，本章强调 **库存侧 API 必须可单独幂等重放**，以便编排器反复补偿。
...
### 22.7.1 与商品中心集成（商品上架时初始化库存）

商品中心在 SKU 生效时发出领域事件（或消息）是最佳挂钩点：库存服务消费事件后生成 `InventoryCreateCommand`，再由创建任务异步初始化 `inventory_config`、`inventory_balance`、券码批次或日期 / 门店切片。完整创建模型见 22.2.6，本节只强调系统边界。
...
**阅读建议**：若读者刚完成第 21 章商品中心与第 6 章一致性章节，可按「商品发布 → 库存创建 → 创单预占 → 支付确认 → 对账修复」的顺序对照本章示意图走读一遍；再把自家品类的库存对象、库存范围、事实来源、单元形态和扣减时机填入矩阵，通常能在工作坊中快速对齐产品与工程预期。建议同时准备 1～2 个真实故障案例作为讨论锚点，避免停留在抽象原则层面。

**下一章预告**：第 24 章将深入营销系统，重点讨论优惠计算与营销库存（券、活动、补贴）如何与商品库存协同，避免「算得便宜却卖超了」这类跨域问题。
