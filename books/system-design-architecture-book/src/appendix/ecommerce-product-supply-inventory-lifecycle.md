...
#### 5.2.3 决策点 3：Redis 是否可以作为库存权威

| 方案 | 优点 | 缺点 / 风险 | 推荐结论 |
| --- | --- | --- | --- |
| 方案 A：Redis 既做热路径，也做权威库存 | 延迟低 | 丢失解释能力；故障恢复困难；漂移后难以自证真相 | 不推荐 |
| 方案 B：Redis 做热视图，MySQL 做权威余额与账本 | 高并发能力和可解释性兼顾 | 要接受热视图重建和最终一致治理 | 推荐 |

结论：

- Redis 不作为库存权威
- Redis 负责热路径扣减和高并发读写
- MySQL 的 `inventory_balance + inventory_ledger` 才是最终解释和恢复的权威基础
...
这一节只回答库存中心的高并发和热点问题，不重复异常修复矩阵。

库存中心的专属技术主题包括：

- Redis 热视图与 MySQL 权威账本的职责分离
- Lua / CAS 的适用边界
- 热点 SKU、热门房态、热门场次的并发治理
- 分片、限流、队列削峰
- Redis 热视图漂移后的重建入口
...
- Redis 负责热路径，不负责最终解释
- 热点治理的目标是“先顶住高并发”，不是“把 Redis 变成权威库存”
- 热视图一旦漂移，最终仍要回到账本和资源事实中恢复
