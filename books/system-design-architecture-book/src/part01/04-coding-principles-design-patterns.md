...
> 用整洁代码、Pipeline、策略模式和规则引擎，把架构意图落到可读、可测、可演进的实现里。

---

## 3.1 复杂业务代码的痛点

第 1 章解决了「系统如何分层、依赖如何向内」的问题；本章解决「**同一层里，代码如何写得可演进、可测试**」。电商的下单、库存、营销是典型的**长流程 + 多分支 + 高频变更**场景，若缺少约束，业务代码会迅速腐化。
...
| **规则与代码绑死** | 运营改文案/门槛就要发版 | 与后续营销系统呼应：需**数据驱动**的规则层 |
...
经验法则：**一屏能读完**（含错误处理）较理想。长逻辑不是「拆成很多小函数」就够了，还要让调用方读出**业务流程**，这正是 8.3 节 Pipeline 的价值。
...
> 生产落地时，可将 `When` / `Apply` 替换为**声明式条件 AST + 有限动作集**，配合配置中心热更新；与后续营销系统衔接时，注意**资损防控**（双写试算、幂等锁券）。
...
**与全书的关系**：前文已经讨论方法论、业务边界、系统内部结构、系统间协作和架构质量保障；本章则把这些架构原则继续下沉到代码层，给出**同层内的组织手法**，让工程纪律真正落到日常实现中。
