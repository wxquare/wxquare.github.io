...
> 用架构评审、设计清单、代码审查和上线验证，守住架构从图纸到生产的质量防线。
...
系统化的 Code Review 不是挑剔，而是**把重构前移到成本最低的阶段**。按 **架构 → 设计 → 代码 → 上线前** 四段清单推进，并与前文的方法论、战略设计、内部结构设计交叉引用，团队可以在一致语言下讨论分层、边界与实现细节。建议将 **7.6.1** 嵌入 MR 模板，并在复盘时根据失效案例增补**第 21 条**——最好的 Checklist 永远是活文档。
