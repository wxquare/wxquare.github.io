...
| 训练目标 | 推荐入口 |
| --- | --- |
| 容量、热点与限流 | [Q-SD-CORE-01](#q-sd-core-01热点交易的预约限流与兑现)、[Q-SD-CORE-09](#q-sd-core-09大促准备容量与降级) |
| 一致性、幂等与恢复 | [Q-SD-CORE-02](#q-sd-core-02跨域创单的一致性边界)、[Q-SD-CORE-03](#q-sd-core-03发布后的读模型一致性) |
| 商品、库存与供给治理 | [Q-SD-CORE-04](#q-sd-core-04商品供给到可售状态的全链路) |
| 搜索、购物车与订单 | [Q-SD-CORE-05](#q-sd-core-05搜索与商品交易事实如何协作)、[Q-SD-CORE-06](#q-sd-core-06购物车与结算的价格库存和实验边界)、[Q-SD-CORE-07](#q-sd-core-07订单状态机与履约编排) |
| 支付、退款与对账 | [Q-SD-CORE-08](#q-sd-core-08支付回调退款与对账) |
| 平台演进与综合案例 | [Q-SD-CORE-10](#q-sd-core-10单体到服务化的演进决策)、[Q-SD-CORE-11](#q-sd-core-11多仓多币种-b2b2c-平台)、[Q-SD-CORE-12](#q-sd-core-12线上数据异常的诊断与恢复) |
...
**题型：** 核心案例，45 分钟
**目标：** 在“活动一分钟内涌入 100 万请求、限量 1 万份、支付可延迟”的约束下，设计从资格校验到库存确认的主链路。
...
1. 库存、预约、订单和支付状态分别由谁权威保存。
2. 入口保护、资格校验和库存令牌分别在哪一层完成。
3. 预约、创单、支付确认和释放的状态机如何收敛。
4. Redis、消息队列和数据库失效时，哪些结果可延迟，哪些必须停止承诺。
...
**题型：** 核心案例，45 分钟
**目标：** 设计商品从供应商同步或运营录入，经审核、发布、库存初始化、营销配置到 C 端可售的链路。
...
1. 各域的事实、控制面和读模型分别是什么。
2. 如何使用发布版本、防覆盖和冻结快照处理并发编辑。
3. 库存创建、券码导入和下游刷新如何任务化、幂等化和补偿。
...
**题型：** 快问快答，5 分钟
**预期结论：** Redis 可承担限流、令牌和热点预扣；锁可减少竞争。但最终承诺仍应由持久化账本、条件更新、唯一约束或状态机决定。
**适用边界：** 当 Redis 只生成可消费令牌时，令牌数必须来自可审计的资源分配，并在故障恢复时与权威账本对齐。
...
**题型：** 核心案例，30 分钟
**目标：** 设计商品检索、筛选、排序和详情展示，处理索引异步更新、价格个性化与无结果场景。
...
1. 检索文档包含哪些可异步字段，哪些字段在交易前必须回源。
2. 如何用版本、重建和灰度保护索引变更。
3. 如何为无结果、低相关性和多语言检索建立可验证反馈。
...
**题型：** 核心案例，30 分钟
**目标：** 设计跨端购物车、结算试算和下单确认，处理价格变化、商品失效和展示实验。
...
1. 状态机按聚合如何拆分，哪些状态允许互相驱动。
2. 超时关单与迟到支付如何仲裁。
3. 拆单、缺货、地址异常和部分退款如何保留可追溯事实。
...
**题型：** 核心案例，45 分钟
**目标：** 设计多渠道支付的支付单、回调、主动查询、退款和 T+1 对账机制。
...
1. 支付、退款和结算的业务唯一键及状态前置条件。
2. 回调、主动查询和对账文件冲突时的证据优先级。
3. 资金差异、金额不符和长时间处理中状态的自动与人工处理边界。
...
**题型：** 核心案例，45 分钟
**目标：** 为一次大促制定容量估算、压测、预热、隔离、限流、降级和恢复方案。
...
1. 峰值假设如何变成缓存、队列、数据库和下游的容量预算。
2. 哪些保护位于网关、应用、数据和运营层。
3. 多机房、依赖故障与异常流量下如何保护交易事实。
...
**题型：** 核心案例，60 分钟
**目标：** 设计包含商家、商品、库存、价格、税费、支付、结算和售后的跨境平台主链路。

**关键决策：** 领域所有权、报价快照、汇率与税费版本、库存分配、支付和结算账本、退款与汇兑差异。
**故障注入：** 汇率刷新滞后、仓库库存回传延迟、支付成功但商家结算暂挂。
**评估锚点：** 能区分交易时承诺、事后结算与用户展示；能解释法律、资金和运营约束何时改变设计。
...
**评估锚点：** 不直接改最终状态掩盖问题；能保留审计、重复保护和可回滚修复。

## 迁移覆盖索引

下表保留旧题号及其主题。`核心案例`表示题目被完整吸收；`约束变体`表示作为指定核心案例的约束变化练习；`快问快答`表示保留其概念判断和适用边界；`参考`表示保留为相应核心案例的补充材料。
...
本场使用 [Q-SD-CORE-02](#q-sd-core-02跨域创单的一致性边界)，并在容量、权威状态和超时重试三个维度各加入一个约束变体。
...
本场使用 [Q-SD-CORE-10](#q-sd-core-10单体到服务化的演进决策)、[Q-SD-CORE-07](#q-sd-core-07订单状态机与履约编排) 和 [Q-SD-CORE-09](#q-sd-core-09大促准备容量与降级)。
...
主案例使用 [Q-SD-CORE-01](#q-sd-core-01热点交易的预约限流与兑现)。递进追问依次引入库存正确性、创单幂等性和异常流量限制三个约束变体。
