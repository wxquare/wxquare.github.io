...
**延伸阅读建议**：读完本章可对照第 28 章营销系统边界与第 32 章订单价格快照设计，把「试算 → 创单 → 收银台 → 支付」四个时间点的**口径表**画在团队 wiki 上，作为跨团队评审的检查清单。实现上新加一层价格或一类费用时，先更新该表，再写代码，能显著降低联调返工。
