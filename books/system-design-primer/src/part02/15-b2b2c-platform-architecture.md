...
## 15.1 项目背景与业务约束（Business Context）
...
## 15.2 品类业务模型分析（Business Architecture）
...
## 15.3 DDD战略设计与系统边界（Application Architecture - 设计过程）

基于15.2的品类业务分析，本节展示如何运用DDD战略设计方法，从业务领域识别限界上下文、划分系统边界、设计服务间集成方式，最终形成15.4的整体架构全貌。
...
以上展示了系统的整体架构全貌：四层架构、12个核心微服务、服务依赖关系、数据流转模式。这些是15.3战略设计的具体落地——12个限界上下文对应12个微服务，上下文映射关系决定了服务间的集成方式。

接下来15.5节将讨论技术选型决策，15.6节将深入各个系统的详细设计。


---

## 15.4 整体架构设计（Application Architecture - 设计结果）

基于15.3节识别的12个限界上下文和上下文映射关系，本节展示如何将它们落地为具体的架构设计：四层架构、微服务拆分、服务依赖关系、数据流转模式。
...
## 15.5 技术选型决策（Technology Architecture）
...
### 15.6 核心系统设计（Application + Data Architecture详细设计）

基于15.4的整体架构，本节深入每个核心系统的详细设计，包括应用层的业务逻辑设计和数据层的模型设计。
...
这四类入口应该进入同一个“供给治理控制面”，共享任务模型、校验、审核、发布版本、Outbox、补偿和可观测性；但供应商同步因为存在长任务、Checkpoint、Raw Snapshot、Worker 租约、DLQ、数据新鲜度等复杂问题，可以单独展开成 `15.6.1.7` 和附录案例。

相关答辩判断已统一收录到[附录 D 的相应核心案例](../appendix/system-design-questionbank.md#q-sd-core-11多仓多币种-b2b2c-平台)。
...
| **供应商同步** | 酒店、影院、活动、票务等外部数据同步 | 同步任务 → Raw Snapshot → 标准化 → 映射 → Diff → 发布 | 接口不稳定、模型不一致、新鲜度、长任务失败 | 独立同步链路，见 `15.6.1.7` |
...
人工创建链路的答辩提示已统一收录到[附录 D 的相应核心案例](../appendix/system-design-questionbank.md#q-sd-core-11多仓多币种-b2b2c-平台)。
...
所以本章采用“主链路 + 专项链路”的写法：`15.6.1.6` 讲统一商品供给、运营与生命周期治理，完整设计见[第 11 章：商品供给、运营与生命周期治理](./11-product-center-supply-lifecycle.md)；`15.6.1.7` 专门讲供应商同步，因为它有长任务恢复、外部数据追溯和供应商质量治理等额外复杂度。
...
从系统边界上看，它属于 `15.6.1.6` 里的供应商供给入口，但执行层要单独设计。统一供给平台负责发布模型、审核、Outbox 和质量治理；供应商同步专项链路负责外部协议适配、Raw Snapshot、Checkpoint、租约、批次版本、新鲜度和供应商质量治理。
...
完整的任务模型、Checkpoint、Worker 租约、DLQ 和监控指标，见[第 11 章：商品供给、运营与生命周期治理](./11-product-center-supply-lifecycle.md)。
...
供应商同步失败治理的答辩提示已统一收录到[附录 D 的相应核心案例](../appendix/system-design-questionbank.md#q-sd-core-11多仓多币种-b2b2c-平台)。
...
供应商同步 DLQ 的答辩总结已统一收录到[附录 D 的相应核心案例](../appendix/system-design-questionbank.md#q-sd-core-11多仓多币种-b2b2c-平台)。
...
供应商同步整体答辩总结已统一收录到[附录 D 的相应核心案例](../appendix/system-design-questionbank.md#q-sd-core-11多仓多币种-b2b2c-平台)。
...
搜索导购的答辩总结已统一收录到[附录 D 的相应核心案例](../appendix/system-design-questionbank.md#q-sd-core-11多仓多币种-b2b2c-平台)。

---

##### 15.6.1.10 跨系统集成与事件设计
...
##### 15.6.1.12 实现落地参考：DDD 分层与代码组织

前面 15.6.1.2 到 15.6.1.11 讨论的是商品中心的业务架构和数据架构。本节保留一个简化版 DDD 代码落地参考，用于说明 Product 聚合根、Repository、接口层、事件订阅和缓存如何组织。它不是上述完整商品中心的全量实现，而是帮助读者理解“架构如何落到代码”的最小示例。

###### 15.6.1.12.1 八层模型的工程落地方式
...
###### 15.6.1.12.5 基础设施层（Infrastructure Layer）

####### 15.6.1.12.5.1 核心存储设计
...
详见下文"15.6.1.12.8 三级缓存实现"。

---

####### 15.6.1.12.5.3 消息中间件（Messaging）⭐️
...
详见 `15.6.1.12.7.4 事件订阅者的分层设计`。

---

###### 15.6.1.12.6 接口层（Interface Layer）

####### 15.6.1.12.6.1 gRPC 接口定义
...
###### 15.6.1.12.7 应用服务层（Application Layer）

####### 15.6.1.12.7.1 核心代码结构
...
###### 15.6.1.12.8 三级缓存实现（Infrastructure Layer 详细实现）
...
| 前置章节 | 在本章的应用 |
|---------|------------|
| **第 1 章**（架构方法论） | Clean Architecture分层、DDD战略设计（15.5.8）、CQRS读写分离 |
| **第 1 章**（领域驱动设计） | 12个限界上下文划分、上下文映射、防腐层（15.6.4） |
| **第 2 章**（代码整洁） | 策略模式（品类策略）、适配器模式（供应商集成）、SOLID原则 |
| **第 7 章**（架构质量保障） | ADR、代码评审清单、测试策略 |
| **第 11 章**（商品中心） | SPU/SKU模型、类目属性、商品快照 |
| **第 12 章**（库存系统） | 二维库存模型、预占机制、超时释放（15.5.2） |
| **第 13 章**（营销系统） | 营销规则引擎、优惠券锁定、最优解求解 |
| **第 11 章**（供给运营） | **商品上架、供应商同步、运营编辑（15.5.6 新增）** |
| **第 13 章**（营销与计价系统） | 四层价格模型、试算接口、快照生成 |
| **第 14 章**（搜索导购内容） | **Query→Recall→Rank→Hydrate链路（15.5.7 新增）** |
| **第 14 章**（购物车结算） | **未登录加购、登录合并、Saga编排（15.5.7 新增）** |
| **第 14 章**（订单系统） | **状态机、Saga模式、幂等性（15.5.7、15.5.8 新增）** |
| **第 14 章**（支付系统内容） | 支付创建、回调处理、对账流程 |
