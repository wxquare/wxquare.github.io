...
### 14.23 决策点 1：为什么 C 端不能只用一个“大前台服务”承接全部链路
...
### 14.24 决策点 2：为什么搜索结果和详情页允许不同一致性口径
...
### 14.25 决策点 3：为什么购物车不锁库存，结算才预占库存
...
### 14.26 决策点 4：为什么订单必须保存商品 / 价格 / 履约快照
...
### 14.28 决策点 6：为什么售后必须基于订单事实，而不是回查最新商品
...
#### 14.32.3 搜索结果页为什么只认 ES 骨架，不直接等于交易事实
...
#### 14.32.4 搜索结果页的 Hydrate 编排为什么不是纯串行，也不是无脑全并行
...
#### 14.32.13 Query、Recall、Rank、Hydrate 的性能边界
...
#### 14.32.17 查询契约要表达用户语义，而不是暴露索引细节
...
#### 14.32.19 Hydrate 的并发控制决定读链路是否会反噬交易链路
...
#### 14.32.20 排序、词典和索引发布要形成可回放的变更流水线
...
### 14.36 场景四：详情页动态 Hydrate 与降级时序图
...
#### 14.37.1 酒店哪些信息放 ES，哪些信息回商品中心 / 报价引擎
...
#### 14.42.1 为什么购物车不锁库存，而要等到结算阶段才预占
...
#### 14.42.3 结算页为什么本质上是一笔短生命周期 Saga
...
#### 14.42.4 结算页为什么必须返回凭证，而不是只返回一个总价
...
#### 14.52.1 订单应该在“提交订单”时创建，还是在“点击支付”时创建
...
#### 14.52.2 订单提交流程应该由独立聚合服务编排，还是由订单中心自己协调下游
...
#### 14.52.4 预生成的订单号能不能直接作为创单幂等 token
...
#### 14.52.5 技术幂等和业务重复单提醒，为什么要分两层设计
...
#### 14.52.6 创单失败后，库存预占和权益占用怎么做最终一致性补偿
...
#### 14.52.7 价格防篡改应该重新计算一遍，还是依赖价格签名 / 版本核销
...
#### 14.52.8 订单快照应该由谁构建：商品中心、订单中心，还是结算服务
...
#### 14.52.9 订单中心负责交易事实编排，商品中心负责交易前静态契约

商品中心负责“商品身份与价格合规”的静态校验，订单中心负责“交易行为与流水合规”的最终编排。

#### 14.52.10 订单必须保存商品快照、价格快照和履约快照

订单必须保存商品快照、价格快照和履约快照，而不是回读最新商品。

#### 14.52.11 支付发起和支付结果回调是两条不同链路

支付发起和支付结果回调是两条不同链路：前者负责创建支付单，后者负责提供支付事实。

#### 14.52.12 支付系统只提供支付事实，订单系统自己推进状态机

支付系统只提供支付事实，订单系统自行推进自己的状态机。

#### 14.52.13 支付成功之后，订单中心再去编排库存确认、权益确认和后续履约触发

支付成功回调之后，订单中心才去编排库存确认、权益确认和后续履约触发。

#### 14.52.14 支付失败、超时取消和风控拒绝后，库存和营销权益必须显式释放

支付失败、超时取消和风控拒绝后，库存和营销权益必须显式释放。

#### 14.52.15 订单模型的演进：从支付单 + 订单，到主单 / 子单 / 商品维度退款
...
#### 14.52.16 为什么库存 Confirm 不应该反查订单中心，而订单中心必须主动查询支付网关
...
#### 14.52.17 Outbox 本地消息表：支付成功后如何可靠驱动库存 Confirm
...
### 14.59 结算服务中的“预占凭证 + 本地事务 + 异步确认”Saga 编排
