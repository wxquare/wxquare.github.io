...
| **供应商同步** | 酒店、影院、活动、票务等外部数据同步 | 同步任务 → Raw Snapshot → 标准化 → 映射 → Diff → 发布 | 接口不稳定、模型不一致、新鲜度、长任务失败 | 独立同步链路，见 `34.6.1.7` |
...
所以本章采用“主链路 + 专项链路”的写法：`34.6.1.6` 讲统一商品供给、运营与生命周期治理，完整设计见[第 25 章：商品供给、运营与生命周期治理](./02-product-center-supply-lifecycle.md)；`34.6.1.7` 专门讲供应商同步，因为它有长任务恢复、外部数据追溯和供应商质量治理等额外复杂度。
...
完整的任务模型、Checkpoint、Worker 租约、DLQ 和监控指标，见[第 25 章：商品供给、运营与生命周期治理](./02-product-center-supply-lifecycle.md)。
...
本章通过一个中大型B2B2C电商平台的完整案例，展示了从业务分析到技术落地的全过程，**是全书知识点的综合实践验证**。本章不仅覆盖了架构方法论（第 1-9 章），还深入展示了**供给运营系统（第 25 章）**和**C端核心交易流（第 30-33 章）**的完整实现，真正做到了"理论→实践→落地"的闭环。
...
| **第 25 章**（供给运营） | **商品上架、供应商同步、运营编辑（34.5.6 新增）** |
| **第 28 章**（营销与计价系统） | 四层价格模型、试算接口、快照生成 |
| **第 36 章**（搜索导购内容） | **Query→Recall→Rank→Hydrate链路（34.5.7 新增）** |
| **第 36 章**（购物车结算） | **未登录加购、登录合并、Saga编排（34.5.7 新增）** |
| **第 36 章**（订单系统） | **状态机、Saga模式、幂等性（34.5.7、34.5.8 新增）** |
| **第 36 章**（支付系统内容） | 支付创建、回调处理、对账流程 |
