...
#### 3.4.5.7 中间件关键配置：Redis 与 MySQL

中间件配置不能只按“默认值能用”来管理。连接池、超时和生命周期参数直接决定系统离故障有多远，也需要像代码一样做评审、压测和灰度。

**MySQL 关键配置**

| 配置 | 含义 | 建议与作用 |
| --- | --- | --- |
| `max_connections` | MySQL 服务端最大连接数 | 上限必须大于所有应用实例连接池之和，并预留监控、迁移和手工运维连接 |
| `wait_timeout` | 非交互空闲连接超时 | 应大于应用侧最大空闲时间，否则连接会被服务端提前关闭 |
| `interactive_timeout` | 交互式空闲连接超时 | 运维客户端连接受此限制，通常比 `wait_timeout` 更宽松 |
| `MaxOpenConns` | 应用侧最大打开连接数 | 决定并发 SQL 上限，过大打满 MySQL，过小限制吞吐 |
| `MaxIdleConns` | 应用侧最大空闲连接数 | 保留合理空闲连接以减少建连开销，不需要追求等于 `MaxOpenConns` |
| `ConnMaxLifetime` | 连接最长存活时间 | 应小于或等于 MySQL/代理空闲断开时间，避免复用失效连接 |
| `ConnMaxIdleTime` | 空闲连接最长保留时间 | 可回收长时间闲置连接，降低应用和 MySQL 资源占用 |
| `connectTimeout` | 建连超时 | 防止 MySQL 不可达时请求无限等待 |
| `readTimeout` | 读取响应超时 | 防止慢 SQL 长期占用应用线程；超时后连接通常需要丢弃，不能无脑复用 |
| `writeTimeout` | 写入响应超时 | 防止网络写入卡死；写超时后必须处理不确定结果，不能直接重试 |
| `max_execution_time` | 单个 SELECT 执行时间上限 | 在数据库侧兜底，防止慢 SQL 长期占 CPU |
| `lock_wait_timeout` | 锁等待超时 | 防止热点行锁把事务拖死 |

**Redis 关键配置**

| 配置 | 含义 | 建议与作用 |
| --- | --- | --- |
| `maxclients` | Redis 服务端最大客户端连接数 | 上限必须大于所有应用实例连接池之和，并预留监控和管理连接 |
| `timeout` | 空闲连接超时 | 配合客户端 `IdleTimeout` 清理闲置连接 |
| `MaxActive` / `PoolSize` | 客户端最大活跃连接数 | 控制单实例并发上限，池满时会排队或等待 |
| `MaxIdle` / `MinIdleConns` | 客户端空闲连接上限/下限 | 保留足够空闲连接，减少 Redis 建连开销 |
| `PoolTimeout` | 获取连接等待上限 | 防止连接池耗尽时请求无限等待 |
| `IdleTimeout` | 空闲连接回收时间 | 避免连接长期闲置占资源 |
| `DialConnectTimeout` | 建连超时 | 防止 Redis 不可达时请求被拖住 |
| `DialReadTimeout` | 读响应超时 | Redis 单线程串行执行命令，慢命令会拖长所有等待者 |
| `DialWriteTimeout` | 写请求超时 | 防止网络或 Redis 故障时命令悬挂 |
| `MaxRetries` | 命令失败重试次数 | 读命令可配置，写命令需评估幂等和放大风险 |

配置评审时至少检查三组关系：

1. 应用连接池总连接数是否小于中间件服务端连接数上限，并预留恢复窗口。
2. 应用连接生命周期是否小于或等于中间件和网络层空闲断开时间。
3. 超时是否覆盖建连、读、写和获取连接，而不是只配一个请求总超时。

常见事故与配置关系：

- MySQL 连接池被打满：通常是慢 SQL、长事务或连接池过大，而不是单个参数写错。
- MySQL 偶发断连：通常是 `wait_timeout`、代理空闲超时小于应用 `ConnMaxLifetime`。
- Redis 请求排队变慢：连接池本身通常不是首要瓶颈，慢命令、大 key、热点 key 更容易放大延迟。
- Redis 获取连接超时：可能是连接池过小、客户端未归还连接，或 Redis 慢命令占满连接。

中间件参数建议先按保守值上线，再通过压测、监控和事故复盘调整。不能只相信官方默认值，也不能把参数改成只看 QPS 不看尾部延迟。
