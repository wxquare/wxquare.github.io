...
Redis 的数据结构、缓存和集群边界可参见[Redis 缓存实践](../part02/02-redis-cache-practice.md)，消息分区、消费和积压治理可参见[Kafka 与消息队列](../part02/03-kafka-message-queue-async.md)，库存权威事实则可参见[库存系统实战](../part03/04-inventory-system.md)。本章关注这些基础设施如何组合成抗洪峰的写入方法论，而不是重新介绍某一个中间件的全部 API。把消息、事务和状态迁移视为可以组合的模式，而不是把某个产品名称当成方案本身，符合企业集成模式所强调的“上下文决定模式”的思路。[5]
