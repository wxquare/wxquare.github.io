# 第20章 个人知识管理 Agent 实践

> "Knowledge is not information storage, it's continuous training." （知识不是信息存储，而是持续训练）—— Andrej Karpathy

## 引言

第17章我们看到了企业级告警诊断 Agent 的复杂性，第18章又把企业知识助手落到 RAG、搜索、权限和治理实践中，第19章则用一个可运行 lab 展示了 Coding Agent Runtime 的最小闭环。但 Agent 不仅适用于企业场景，也适用于个人生产力。
...
| 指标 | 人工基线 | Agent 辅助流程 | 评估口径 |
|------|------|------|------|
| **笔记数量** | 50 篇 | 150 篇 | 同一周期内进入 `wiki/` 的可复用笔记数量 |
| **概念提取** | 约 2h/篇 | 约 5 min/篇初稿 + 人工抽查 | 从原文生成摘要、概念、标签和双链的端到端时间 |
| **检索时间** | 5-10 分钟 | 约 30 秒 | 从提出问题到找到可引用笔记的平均耗时 |
| **知识连接** | 手动维护 | 自动生成 + 人工确认 | 新笔记与已有概念之间的候选链接生成比例 |
| **笔记复用率** | 约 20% | 约 80% | 被问答、文章、演讲稿或复盘再次引用的笔记比例 |
...
| **实时性** | 无要求 | 秒级响应 |
| **成本** | 取决于模型、token、缓存和使用频率 | 取决于告警量、模型、工具和集成成本 |
| **价值** | 个人生产力 | 团队效率 |
