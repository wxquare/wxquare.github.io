# 第24章 LLM 推理机制：Prefill、Decode、Sampling 与 KV Cache

LLM 推理不是简单的“跑一次模型”。对自回归模型来说，生成文本是一个循环：每次预测下一个 token，把它追加到上下文，再继续预测下一个 token。

本章先从宏观上理解推理流程，再看工业界如何优化 serving，最后看 KV cache、speculative decoding、paged memory、量化和长上下文相关研究。

## 24.1 宏观理解：一次生成是怎么发生的
...
## 24.7 工业实践：Serving Engine 在优化什么
...
## 24.8 PagedAttention 与 Paged KV Cache
...
## 24.9 Continuous Batching 与 In-flight Batching
...
## 24.10 Prefix Reuse 与 Prompt Caching
...
## 24.17 深入理解：Prefill 和 Decode 的资源画像不同
...
## 24.18 深入理解：为什么 Decode 经常是 Memory-bound
...
## 24.20 深入理解：Prefix Cache 不是普通缓存
...
## 24.21 研究补充：Reasoning 模型放大了 KV Cache 问题
