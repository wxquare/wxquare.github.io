# 第6章 Agent 工具系统工程：Tool Calling、Skills 与 MCP
...
## 6.1 Tool Calling 决策：从函数调用到受控行动

### 6.1.1 从问题出发：为什么不是直接 API 调用
...
### 6.1.3 Function Calling、Tool Calling、Skills 与 MCP 的边界
...
MCP 不替代 Tool Runtime。它提供“工具能力如何被发现和调用”的协议边界，但工具插上去之后能不能安全可靠地工作，仍取决于 Schema、Policy、Sandbox、Trace 和 Eval。

### 6.1.4 Tool Calling 是运行时系统，不只是模型 API 功能
...
### 6.1.5 工具接入方式选择框架：API、CLI、MCP 与 Browser Use
...
## 6.2 工具契约：Schema、返回结果与能力暴露

### 6.2.1 工具 Schema 是模型与系统之间的契约
...
### 6.2.4 Schema 设计原则：名称、参数、幂等、边界与错误
...
### 6.2.5 ToolResult Envelope：把模型上下文和系统元数据分开
...
## 6.3 Tool Runtime：生产级工具调用的控制平面

### 6.3.1 最小 Tool Runtime 心智模型
...
### 6.3.2 Tool Runtime 核心组件：Registry、Validator、Policy、Executor 与 Trace
...
### 6.3.3 工具调用状态机：从 Requested 到 Observed
...
### 6.3.4 失败模式：误选工具、参数错误、重复副作用与注入攻击
...
### 6.3.6 可观测性指标：成功率、延迟、审批率、成本与上下文预算
...
### 6.4.1 为什么 Tool Calling 还需要 Skills
...
### 6.4.2 Skill 的定义：触发条件、步骤、工具、约束与验证
...
### 6.4.3 Tool、Skill、Workflow、Memory、Plugin 的边界
...
### 6.4.6 Skill Registry：技能也需要版本、Owner 和风险等级
...
### 6.4.7 Skill Selection：不是每次全部加载
...
### 6.4.9 Skill 生命周期：Create、Review、Validate、Publish、Retire
...
## 6.5 MCP：Tool Calling 的标准化接入协议

### 6.5.1 为什么有了 HTTP、REST 和 OpenAPI 还需要 MCP
...
### 6.5.3 MCP 与 API、CLI、Browser Use 的边界
...
### 6.5.4 MCP 的架构边界：Host、Client、Server
...
### 6.5.5 MCP 的能力模型：Tools、Resources、Prompts
...
### 6.5.6 Capability Negotiation：能力协商与渐进兼容
...
### 6.5.7 MCP 协议流：initialize、list、call、read 与 notifications
...
### 6.5.8 stdio 与 Streamable HTTP：本地 Server 和远程 Server
...
### 6.5.10 GitHub 与 Log MCP 示例：API、CLI 与 MCP 的三条路径
...
### 6.5.11 MCP 不是 API Gateway 的替代品
...
### 6.6.1 Sandbox 的职责：限制文件、网络、进程和凭据边界
...
### 6.6.2 Sandbox 不是什么：不是 Prompt、审批、Docker 或 API RBAC
...
### 6.6.4 执行环境的四种层级：Path、Process、Container、Remote Runner
...
### 6.6.6 Sandbox Regression Tests：文件、网络、凭据与 MCP Server 测试
...
## 6.7 工具编排模式：从单次调用到可治理流程

### 6.7.1 Direct Tool Calling
