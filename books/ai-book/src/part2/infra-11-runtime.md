# 第11章 Agent/模型运行时 Infra：调度、工具、状态与多租户
...
Runtime 的外部 API 至少分成 task submit、run query、event stream、approval、cancel、resume、artifact 和 replay。提交接口接受目标、上下文、能力范围、预算、deadline、优先级和幂等键；查询接口返回当前状态、进度、成本摘要、需要的动作和可见结果；事件流提供状态变化但不暴露未授权内容。接口版本和字段兼容性要独立于内部 workflow 版本。
...
事件订阅至少提供 at-most-once、at-least-once 或基于 offset 的消费语义。UI 可以接受丢失中间进度，计费和审计不能丢失最终状态；审批和工具回调需要可重放。订阅者保存 cursor，重新连接后从 cursor 读取；事件带 schema version、sequence 和 timestamp，旧消费者通过兼容转换读取新事件。

事件本身不等于状态。消费者可能延迟或重复，Runtime 的查询接口从持久化状态返回当前事实；事件用于实时更新和回放。客户端收到 TOOL_SUCCEEDED 但查询仍显示等待时，应以带版本的状态查询和最终一致规则处理，而不是把事件当作唯一真相。明确这点可以避免前端、计费和审计各自维护一套状态。

### 版本与迁移

工作流和状态 schema 的升级需要 migration plan。新增可选字段可向后兼容；改变状态含义、节点 id、工具参数或补偿语义需要新版本与迁移函数。长时间运行的 Run 可能跨多个版本，控制面保存每一步的 workflow version，而不是用当前 latest 解释历史。迁移工具先在复制的 state 上演练，再处理生产 Run。

迁移失败时回到旧版本还是暂停人工，取决于是否已经执行不可逆动作。状态转换保存 before、after、migration version 和 operator。禁止直接修改数据库字段绕过迁移，因为事件重放、审计和回滚会失去一致性。数据库 schema、事件 schema、artifact schema 与 API schema 分开版本管理，但由同一 release 记录关联。

### 工具的生命周期

工具从注册、审核、测试、灰度、可用、暂停到退役。注册时检查 owner、来源、代码签名、依赖、schema、权限、数据流、side effect、超时、限流和补偿；测试时使用 sandbox 与恶意输入；灰度按租户和任务；退役前迁移已有 Run 或让其明确失败。MCP server 的发现能力不能跳过这个生命周期。

工具暂停可以由错误率、延迟、权限变更、安全事件或外部维护触发。暂停后的新调用快速失败或选择兼容 fallback，正在执行的调用由工具 policy 决定是否等待。状态中记录暂停 reason、policy version 和恢复时间。工具列表从注册表动态读取时，Run 仍锁定已选择的版本，避免中途能力变化。

### 模型路由与 Runtime

Runtime 的模型路由比普通 serving 多一层任务语义。规划、摘要、分类、工具参数、验证和最终回答可能适合不同模型；路由依据任务类型、质量、延迟、成本、上下文、租户和安全等级。模型能力声明包括支持的输入模态、结构化输出、工具、最大上下文、可靠性和质量门禁。

一次 Run 可以跨多个模型，但每个 step 绑定 model revision 和 evaluator evidence。模型不可用时，fallback 需要重新检查能力和预算；不能因为接口相同就认为输出语义相同。路由策略自身版本化，线上 trace 记录候选、选择、拒绝原因、降级和成本。这样可以把一个任务的失败归因到规划模型、工具模型、主模型或路由。

### Context budget 与成本预算

Run 总预算分成输入 token、输出 token、模型调用数、工具调用数、并行度、存储、网络和 wall-clock。每个 step 预留上限，完成后结算实际；父 Run 为子任务设置 ceiling，子任务不能通过无限递归扩大预算。上下文 builder 根据剩余 token 和任务优先级选择历史、记忆、检索和工具结果。

达到预算阈值时，Runtime 可执行策略包括压缩上下文、减少候选、降低模型、停止可选分支、等待人工或返回 partial result。每种策略都写入事件和响应，不应静默改变用户目标。预算不足与模型失败是不同错误，调用方需要不同处理。成本报表按 task、run、step、model、tool 和 tenant 对账。

### 记忆写入的确认协议

长期记忆写入属于持久副作用，不能因为模型说“请记住”就直接提交。Runtime 提取候选事实，标记来源、置信度、敏感等级、过期和主体；策略决定是否需要用户确认、人工审核或直接拒绝。提交使用幂等 key 和版本，冲突产生新版本或进入人工，不覆盖已有事实。

记忆读取也需要最小权限。当前用户可以读取自己的偏好，但不一定能读取组织内部记忆；一个 Agent 的工作区记忆不应自动泄露到另一个租户。删除和更正写入事件，缓存、embedding 和派生摘要按血缘清理。记忆作为上下文来源时，trace 记录 memory id 和版本，不保存不必要的完整内容。

### 多 Agent 协作

多 Agent 运行时将一个主 Run 拆成多个角色或子 Agent，每个子 Agent 有能力、上下文、工具和预算。AutoGen 的对话编排展示了多 Agent 协作的可能性 [5]，但生产系统需要明确 coordinator、worker、reviewer、human 和 join。子 Agent 的输出是候选 artifact，不应自动覆盖主状态；协调者检查 schema、证据和权限后再合并。

子任务共享多少上下文、是否共享记忆、是否允许直接通信、失败是否重试和谁拥有副作用，都要在 workflow 定义中声明。自由的 Agent-to-Agent 消息可能造成循环、信息泄露和成本失控。每条消息有 parent、sender、recipient、purpose、budget 和 trace；并发子任务有上限和 deadline。
...
模型响应是一个候选结果，业务状态是经过规则、工具和事务确认后的事实。Runtime 将 assistant message、tool plan、tool result 和 business commit 分开存储。模型说“订单已取消”不代表订单系统已经成功更新；只有工具返回带有订单版本和确认状态，业务状态才可以标记为已取消。这个边界能避免语言看起来成功而外部事实失败。

业务系统返回的版本、etag 或状态 token 进入 operation state，重复提交先比较版本。若外部状态已经被其他系统改变，Runtime 进入 conflict 或 needs_attention，不让模型继续猜测。对读操作可以重试并刷新观察，对写操作优先查询和补偿。Saga 的局部事务与补偿思路适合解释这种模型—业务状态分离 [18]。

### 计划验证器

复杂任务可在执行前运行 plan validator，检查节点是否存在、依赖是否闭合、工具是否有权限、预算是否足够、循环是否有出口、不可逆动作是否审批、输入输出 schema 是否兼容。validator 可以是规则程序，也可以辅以模型，但最终约束由程序决定。验证失败返回结构化问题列表，模型只能在允许的范围内修正。

执行中每个 observation 触发 postcondition 检查，例如文件是否存在、测试是否通过、数据库状态是否改变、引用是否指向证据。postcondition 失败不一定重试同一个动作，可以重规划、回滚、人工或终止。将前置和后置条件显式化，能把 Agent 的自由度限制在可验证状态图内。

### 工具结果的证据等级

不同工具结果可信度不同。数据库事务确认和签名 API 返回属于高可信事实；搜索摘要、网页内容和模型推断属于低可信观察；用户口述和历史记忆需要来源与时间。Runtime 为 result 记录 provenance、timestamp、source、confidence、version 和 freshness，context builder 按任务要求选择证据等级。

生成最终答案时，要求关键结论绑定 evidence reference；没有证据时输出不确定或请求工具，而不是填补空白。工具结果过期、权限变化或来源撤回时，记忆与 cache 失效。评估平台可以根据证据引用和事实正确建立回归样本，形成运行时与评估的闭环。

### 事件溯源与查询模型

Event sourcing 适合保存运行时事实，projection 适合提供当前 Run、租户账单、工具状态和审计查询。事件追加后由消费者更新 projection，消费者重复处理不会产生不同结果。查询端允许最终一致，但 approval、billing 和 safety 等关键状态需要明确读取最新版本或使用强一致路径。

事件数量大时，按 Run、tenant、时间和 workflow 分区，冷热归档。事件 schema 版本化，老事件通过 upcaster 转换；删除敏感内容可以删除 payload 但保留不可识别的 tombstone 与审计。回放必须能够重建状态，同时不触发真实外部副作用。只存最终 JSON 会丢失重试、拒绝、人工和补偿证据。

### 调度器的公平队列

Runtime 的队列不只排模型请求，还排工具、审批、检索、回调和补偿。不同队列共享 Run budget，但资源瓶颈不同。模型队列看 token 和 GPU，工具队列看连接和第三方配额，审批队列看人工容量，补偿队列看风险和 deadline。全局调度器用 priority、fair share 和 dependency ready 选择工作。

公平队列要防止一个 Run 创建大量 child steps 侵占资源。父 Run 和租户设置并发上限，子任务使用 token bucket 或 semaphore；达到上限后任务等待并在 trace 中显示原因。优先级提升需要审计，不能让调用方通过反复提交高优先级绕过配额。Borg 的配额和优先级经验在此同样适用 [10]。

### 回调与外部系统一致性

外部系统的 webhook 可能重复、乱序、延迟或伪造。Runtime 用签名、source、correlation id、event version、timestamp 和 idempotency key 验证；未知事件存入隔离队列，人工或适配器处理。回调到达时先查询当前 operation 和 Run 状态，再决定完成、重试、忽略或人工。

如果外部系统没有可靠 webhook，Runtime 使用轮询和 deadline，但轮询要有退避、租户配额和停止条件。外部状态查询成功后才能关闭等待 step，不能只因为 callback timeout 就重复创建资源。每种集成在工具注册表声明事件语义、查询接口、最终一致窗口和补偿方式。

### 人工队列的运营模型

人工接管不是一个按钮，而是一个有 SLA、优先级、角色、上下文和审计的队列。任务卡片展示目标、模型计划、证据、风险、待决策选项、预计影响和过期时间，隐藏不必要的隐私。人工修改、批准、拒绝和转交都有事件，超时根据策略暂停、升级或安全失败。

人工结果反馈到 Runtime policy 和评估集，但不能直接当作模型真值。标注指南、角色和理由写入样本；高风险任务需要双人或审批链。人工工作量与自动成功率、误操作和成本一起评估，才能判断是否值得扩大自动化。
...
容量模型需要估计 active Run、ready step、waiting step、模型 token、工具 QPS、事件写入、状态存储、artifact、trace 和人工队列。一个低 QPS 的长 Run 可能占用大量 durable state 和 timers；一个高 QPS 的短任务可能占用模型与 tokenizer；一个工具风暴可能打爆第三方而非 GPU。容量按资源类型分别计算并留出故障余量。

调度器在接纳新 Run 时检查全局和租户预算、依赖、模型队列、工具配额和人工容量。即使 GPU 还有余量，工具或审批不足也应背压。容量 dashboard 显示 planned、reserved、active、blocked、failed 和 retrying，帮助团队区分需求不足、资源不足和策略阻塞。
...
模型输出是概率分布中的采样，不是数据库事实；记忆是带来源和置信度的候选，不是绝对真相；评估分数是对目标分布的估计，不是永久能力。机器学习教材关于泛化、偏差和不确定性的讨论支持这些边界 [22]；神经网络与深度学习教材和动手学深度学习的内容帮助理解表示、序列和训练误差如何影响运行时输入输出 [23][24]。

因此 Runtime 用 schema、状态机、策略、证据和外部确认把不确定性包起来。它不试图消除模型不确定性，而是把不确定性转化为可观测、可暂停、可重试、可人工接管和可回滚的系统状态。这个判断是 Agent Infra 与单纯 prompt 编排之间最核心的区别。

### 运行时配置的不可变性

Run 创建时锁定 workflow、模型路由、工具 allowlist、policy、memory schema、budget、deadline、tenant 和 data region。运行中可以产生事件和临时 observation，但不能无审计地修改这些输入。用户想改变目标、增加预算或允许新工具时，生成新的 revision 或 approval event，旧状态仍可回放。不可变配置让失败复盘知道当时到底执行了什么。

配置中心提供 latest 只是开发便利，生产 Run 必须解析为具体版本。工具描述、prompt 模板和安全规则的热更新不能悄悄影响已在执行的任务；新 Run 使用新版本，长 Run 按兼容迁移规则切换。配置 hash 进入 trace、计费和 evidence，发布控制器根据 hash 判断是否需要重新评估。

### 状态存储的选择

Run 状态适合事务数据库或强一致 KV，事件适合追加式日志，消息和大型观察适合对象存储，记忆和检索使用专门索引，指标使用时间序列，trace 使用分布式追踪系统。一个数据库包办所有内容会让事务、查询、TTL、权限和成本彼此冲突。平台通过稳定 id、版本和引用连接这些存储，而不是复制完整文本。

状态存储需要考虑租户分区、冷热数据、备份、故障恢复和删除。热 Run 查询低延迟，历史 Run 归档到低成本存储；敏感 payload 可以独立加密和短期保留，审计事件保留更久。恢复演练验证 Run 状态、事件、tool operation、memory reference 和 cost event 能否一致重建。

### 幂等键的层级

一个系统至少需要 task idempotency、run idempotency、step attempt idempotency、tool operation idempotency 和 billing event idempotency。task key 防止用户重复提交，run key 区分一次执行，step key 防止 worker 重试重复完成，operation key 防止外部副作用，billing key 防止重复计费。把一个 request id 复用到所有层，会在并行或重试时产生冲突。

幂等键的生命周期、作用域和过期要写入契约。相同 key 不同 payload 应报冲突，不能默默覆盖；key 过期后旧重试应返回明确错误；跨租户 key 不能碰撞。外部系统不支持幂等时，Runtime 使用查询、锁、去重表或强制人工。Idempotent Receiver 模式只是消息层基础，业务语义仍需工具实现 [19]。

### 重试与惊群

多个 Run 同时遇到服务短暂故障时，立即重试会放大故障。Runtime 使用指数退避、随机抖动、租户和工具限流、全局熔断与 retry budget。服务恢复后采用逐步放量，避免所有待重试任务同时冲入。熔断期间，纯读取可以缓存或降级，写操作进入待处理或人工，模型调用可以转小模型但需能力检查。

重试指标包含原始失败、attempt、等待时间、最终成功、重复副作用和额外成本。系统不能把 retry success 当作一次普通 success，否则可用性看似良好但成本与延迟已经失控。错误码区分 retryable、non-retryable、unknown outcome 和 human required，调用方按契约行动。

### 资源泄漏与清理

Run 终止时需要清理模型连接、工具 lease、审批、临时文件、memory draft、检索 cursor、事件订阅、timer、GPU request、trace buffer 和成本 reservation。清理流程按资源所有者执行并可重复；一个 worker 迟到不能删除另一个 Run 的资源。临时资源命名带有 tenant、run、attempt 和随机 token，清理器用归属 manifest 校验。

周期性 reconciler 扫描“状态已终止但资源仍存在”和“资源存在但没有有效状态”的孤儿。reconciler 先标记、等待、验证，再释放；高风险工具和外部 operation 不能仅凭本地超时删除。清理指标包括孤儿数量、平均存活、失败次数和人工干预，避免隐性成本长期累积。

### 运行时的负载测试

负载测试包含短问答、多步检索、工具写入、并行子任务、等待回调、人工审批、长上下文和失败重试。每种 workload 定义到达率、任务长度、工具分布、模型路由、租户比例、deadline 和结果质量。测量 active Run、step queue、状态写入、event lag、tool QPS、模型 token、成本、p95/p99 和最终成功。

测试不只增加请求数，还注入事件乱序、worker 重启、消息重复、工具超时、模型限流、状态库延迟和网络分区。验证任务不会死锁、无限重试、重复副作用或跨租户泄漏。压测结果进入容量模型和 SLO，失败场景进入故障演练与回归集。

### 多租户的公平与隔离证明

隔离验收使用带唯一标记的 prompt、memory、artifact、tool result 和 trace，随机让租户发生失败、重试、取消和恢复，检查其他租户是否可见。资源公平验收让一个租户制造长任务和工具风暴，观察其他租户的 p95、配额和成功率。高优先级可以改善 deadline，但不能无限侵占全局容量。

权限证明不只看正常 API。测试越权读取、错误 tenant header、伪造 callback、复用 operation key、通过 prompt injection 请求工具、通过错误日志读取秘密、通过 cache hit 推断内容。策略版本、拒绝原因和审计证据都保存。OWASP 风险清单提供威胁分类，但具体边界由业务资源和工具定义 [25]。

### 任务成功定义

Runtime 需要让业务 owner 定义 outcome contract。知识查询可能要求答案有引用且没有高风险幻觉；代码修改要求测试通过、diff 在范围内且没有秘密；告警处理要求状态更新、通知和审计完成；数据抽取要求 schema 合法、覆盖率和错误率达标。模型生成文本只是中间产物，不是 outcome。

Outcome evaluator 可以是规则、外部系统确认、测试、人工或组合。结果写入 Run，失败原因分为 model、tool、data、policy、resource、human 和 unknown。成功定义变化时创建新 workflow/evaluator version，不重写旧 Run。评估平台消费这些 outcome，运行时用它们调整路由和重试。

### Runtime 的数据流与反应式系统

运行时事件可看成一个反应式系统：输入事件触发状态读取、策略决策、模型/工具动作和新事件。每个 handler 纯化一部分逻辑，副作用通过 command 和 operation 管理。事件处理器必须可重入、可重放、可观测，不能把不可持久的内存状态当作唯一事实。这样在故障后可以从事件和快照重新计算 projection。

反应式处理也要控制递归。工具回调触发模型，模型又触发同一个工具，可能形成事件环。每个 event 带 cause、depth、parent 和 budget，Runtime 检测重复指纹和最大深度。跨服务事件使用 schema registry，未知版本进入兼容层或隔离队列。状态变化必须有清晰 owner，避免两个 handler 互相覆盖。

### Runtime 的可迁移性

把模型调用、工具调用、状态、审批和观测通过内部接口隔离，未来可以替换模型供应商、serving engine、workflow framework 或 MCP server。适配层统一 request、response、error、usage、trace、cancel 和 capability；业务 workflow 不直接依赖某个 SDK 的异常类或字段。LangGraph、Temporal、Ray 和自研状态机可以在不同层发挥作用，但外部 Run 契约保持稳定 [6][7][8]。

可迁移性不是把所有能力降到最低，而是把平台特有能力放在适配层。比如某 engine 支持 prefix cache，Runtime 通过 capability 宣告并将收益纳入指标；某工作流支持 durable timer，Runtime 使用它而不假设所有 backend 都有。迁移前运行兼容测试、回放、故障和成本基线，确认不是仅接口兼容。

### 运行时的最小风险原则

自动化只覆盖可验证、可逆、可限额的动作；不可逆、高风险、外部影响大的动作需要审批或强业务 API。模型可以提出多个候选，策略选择一个或要求人工；工具可以返回事实，Runtime 判断是否足够；工作流可以继续，budget、deadline 和安全可以阻断。让系统在不确定时暂停，比让模型凭语言流畅性继续更安全。

这个原则不是降低 Agent 能力，而是把能力放在可证明的边界内。随着评估、回放、工具可靠性和任务成功证据增加，可以逐步扩大自动化范围；每一次扩大都伴随新门禁和回滚。后端工程师熟悉的渐进发布、熔断、补偿和审计，正是把 Agent 从 demo 推向生产的基础。

### 运行时的变更评审

变更评审至少回答五个问题：状态是否增加或改变，副作用是否增加，失败后的恢复是否仍然安全，租户与权限是否改变，评估与观测是否覆盖。仅增加一个 prompt 字段也可能改变 token、路由、工具选择和隐私边界；仅更换一个 SDK 也可能改变错误与重试。变更单引用 workflow、tool、model、policy、evaluator 和 schema 版本，评审人根据影响范围批准。

小变更可以自动测试，跨边界变更必须做 replay、sandbox、压力和安全。评审结果不是“通过代码”而是“获得什么能力、牺牲什么能力、接受什么风险、如何监控、何时重新评估”。这类 ADR 思维特别适合 Runtime，因为它同时连接产品目标、模型不确定性和分布式状态。

### 运行时的故障预算

任务失败、工具失败、人工接管、超预算、重复副作用、越权和数据泄漏都应进入不同的错误预算。模型偶发格式错误可以通过有限修复消化，支付重复或跨租户泄漏则不能用重试掩盖。平台按错误类别、租户、workflow 和版本统计，触发暂停实验、降低自动化或强制人工。

错误预算不只约束线上可用性，也约束复杂度引入。新多 Agent 策略若成功率略升却让重试和成本大幅增加，可能消耗更多预算；新工具若减少人工但增加安全事件，不应默认上线。Runtime 用任务级证据帮助业务在质量、成本和风险之间做明确选择。
...
若这些证据完整，Runtime 的内部框架可以演进，业务依赖的是稳定的任务和状态契约；若证据缺失，即使 demo 成功也无法在节点故障、工具变化、模型升级或租户增长后安全扩展。这个交付标准也为最后一章可靠性与治理 Infra 提供输入。

最终验收应同时检查正常完成、异常恢复、权限拒绝、租户隔离、成本对账和状态回放；任何一个只在 happy path 上成立的能力，都不能被视为生产运行时能力。

当模型、工具、工作流和策略都具备版本，运行时才能在每次变更后重新评估影响；当事件、状态和副作用都具备幂等，系统才能在重试和恢复中保持业务正确；当预算、SLO 和审计都具备统一标识，平台才能把 Agent 的开放性纳入可运营的边界。

这份统一标识还应贯穿日志、trace、指标、计费、评估和发布记录，使一次任务的技术路径、业务结果和治理决定可以在同一条证据链上互相验证。

只有这种端到端证据，才能支撑运行时的自动化扩大、故障后的安全恢复以及不同租户之间的公平服务。

运行时章节的验收由此闭合：从任务进入、模型候选、工具执行、状态持久化到最终业务结果，每个阶段都有版本、权限、预算、重试、观测和回滚边界；从任意故障恢复后，历史事实不被覆盖，副作用不被重复，新的执行仍然能够被评估和审计。

这比单次演示中的“Agent 完成任务”更严格，也更接近生产系统真正需要承担的责任。
