...
## 10.3 Query Planning：把问题拆成可检索任务
...
## 10.4 Multi-hop Retrieval：跨证据链路检索
...
## 10.6 Tool-Augmented Retrieval：检索不只来自文档
...
## 10.7 Self-Verification：让 Agent 证明答案有依据
...
Agentic RAG 的 Release Gate 应该使用这些 trace 级指标，而不是只看最终答案分数。常见阻断条件包括：

- 缺少必需 hop，例如没有查询当前指标就给出根因；
- 高置信 claim 没有 citation；
- 把历史案例当成当前事实；
- 在证据不足时没有停止；
- 建议高风险动作但没有触发审批；
- 检索调用、token 或延迟超过预算。

这类门禁能防止“答案看起来像对的，但过程不可接受”的版本进入生产。
