...
flowchart LR
    ...
    subgraph Chapters["后续章节地图"]
        ...
        C7["第7章<br/>Agent 知识系统"]
        C8["第8章<br/>Agent 记忆系统"]
        C9["第9章<br/>执行编排与平台架构"]
        C10["第10章<br/>Evals / Guardrails / Observability"]
    end
...
| 核心组件 | 主要职责 | 后续展开 |
|:---|:---|:---|
| Event & Intake Router | 接收聊天、API、告警、工单、Webhook、定时任务等入口 | 第9章工作流、第17章 DoD Agent |
| Intent Normalizer | 把模糊输入变成结构化任务契约 | 第9章入口路由、第10章输入治理 |
| Task Planner | 生成可执行、可验证、可修订的计划 | 第9章执行编排 |
| Context Builder | 组织本轮任务需要的证据和上下文 | 第7章 Agent 知识系统 |
| Memory Layer | 管理跨会话偏好、经验、历史任务和长期上下文 | 第8章记忆系统 |
| Execution State & Checkpoint | 管理任务状态、暂停、恢复、重试、回放和幂等 | 第9章状态机、第8章记忆系统 |
| Capability Registry | 统一管理 Skills、Tools、Connectors、MCP、Prompt 和 Workflow | 第6章工具系统、第9章平台架构 |
| Policy Engine & Human Control Plane | 管理权限、风险、审批、接管、降级和回滚 | 第6章工具权限、第10章 Guardrails |
| Agent Loop | 推动观察、决策、行动、修复和停止 | 第9章工作流与平台运行时 |
| Model Router & Handoff Manager | 管理模型选择、专家委派、多 Agent 协作和跨 Agent 通信 | 第9章多 Agent 与平台架构 |
| Verifier & Eval Harness | 运行时验证和离线回归评测 | 第10章 Evals |
| Review Surface、Trace & Audit | 提供可审查输出、过程追踪和审计证据 | 第10章可观测性、第17章实战案例 |
| Learning Loop | 把反馈、失败案例和复盘经验转化为能力演进 | 第8章记忆系统、第10章 Evals、第16章 Hermes |
...
### 5.4.2 ReAct、Plan-and-Execute 与 Plan mode：三个容易混淆的概念
