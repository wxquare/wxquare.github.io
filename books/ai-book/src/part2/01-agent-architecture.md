# 第11章 Agent 架构总纲：边界、Runtime 与模式选择
...
## 11.1 Agent 架构决策：先判断是否需要 Agent

### 11.1.1 从问题出发：为什么不是传统后端
...
## 11.2 Agent Runtime：生产级 Agent 的最小骨架

### 11.2.1 最小 Runtime 心智模型
...
### 11.2.3 Runtime 初始化：模型、工具、技能、策略与数据源
...
### 11.3.1 Event & Intake Router：Agent 的入口不只是聊天
...
### 11.3.2 Intent Normalizer：把用户输入变成任务契约
...
### 11.3.4 Context Builder：上下文是信息架构，不是拼 Prompt
...
第 7 章会展开 Agent 知识系统中的 RAG、Agentic RAG、MCP Resource 和 Web Search，第 8 章会专门展开 Memory 的读取、写入、遗忘、污染防控和评估。

---

### 11.3.5 Memory Layer：跨会话状态、经验与长期上下文
...
### 11.3.6 Execution State 与 Checkpoint：让 Agent 可暂停、恢复与回放
...
第 9 章讲工作流和状态机时，会把 Execution State 作为核心对象；第 8 章讲 Memory 时，会进一步区分短期会话状态、任务状态和长期记忆。

---

### 11.3.7 Capability Registry：Skills、Tools、Connectors 与 MCP
...
### 11.3.8 Policy Engine 与 Human Control Plane：权限、审批、接管与降级
...
### 11.3.10 Model Router 与 Handoff Manager：模型选择、专家委派与多 Agent 协作
...
第 9 章会展开多 Agent 协作、状态机和平台框架如何支持 Handoff、Agent Team、A2A 和跨系统协作。

---

### 11.3.11 Verifier 与 Eval Harness：运行时验证与回归评测
...
### 11.3.12 Review Surface、Trace 与 Audit：可审查的交付界面
...
### 11.3.13 Learning Loop：从反馈到能力演进
...
### 11.4.2 ReAct、Plan-and-Execute 与 Plan mode：三个容易混淆的概念
