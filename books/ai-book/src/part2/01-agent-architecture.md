# 第8章 Agent 架构总纲：边界、Runtime 与模式选择
...
## 8.1 Agent 架构决策：先判断是否需要 Agent

### 8.1.1 从问题出发：为什么不是传统后端
...
## 8.2 Agent Runtime：生产级 Agent 的最小骨架

### 8.2.1 最小 Runtime 心智模型
...
### 8.2.3 Runtime 初始化：模型、工具、技能、策略与数据源
...
### 8.3.1 Event & Intake Router：Agent 的入口不只是聊天
...
### 8.3.2 Intent Normalizer：把用户输入变成任务契约
...
### 8.3.4 Context Builder：上下文是信息架构，不是拼 Prompt
...
### 8.3.5 Memory Layer：跨会话状态、经验与长期上下文
...
### 8.3.6 Execution State 与 Checkpoint：让 Agent 可暂停、恢复与回放
...
### 8.3.7 Capability Registry：Skills、Tools、Connectors 与 MCP
...
### 8.3.8 Policy Engine 与 Human Control Plane：权限、审批、接管与降级
...
### 8.3.10 Model Router 与 Handoff Manager：模型选择、专家委派与多 Agent 协作
...
### 8.3.11 Verifier 与 Eval Harness：运行时验证与回归评测
...
### 8.3.12 Review Surface、Trace 与 Audit：可审查的交付界面
...
### 8.4.2 ReAct、Plan-and-Execute 与 Plan mode：三个容易混淆的概念
