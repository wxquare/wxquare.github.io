...
大模型的“推理”不是一个单独的神经网络模块，而是模型、提示、搜索、验证器和推理预算共同形成的过程。自回归模型每次只预测下一个 token，但复杂任务需要先拆解问题、保留中间状态、比较候选路径、检查结果，再决定是否继续。于是，推理算法要回答两个问题：如何从概率分布中生成更好的轨迹，如何用额外计算换取更高的任务成功率。
...
## 4.8 长上下文、KV Cache 与算法—系统接口

### Prefill、Decode 与推理阶段

自回归推理通常分为 prefill 和 decode。Prefill 并行处理输入 token，构建各层 attention 的 Key/Value；decode 每次生成一个 token，并复用历史 KV cache。KV cache 让模型不必反复计算历史 token，但显存大致随层数、KV head 数、序列长度、批量和精度线性增长：

$$
M_{KV}\approx 2L\times B\times T\times H_{KV}\times d_{head}\times bytes.
$$

这不是推理算法正确性的核心，却直接决定 test-time scaling 能否落地。采样十条候选、保留多个搜索分支，会同时增加序列和 KV cache；因此算法设计必须给出候选数、最大长度、终止条件和共享前缀策略，Infra 才能估算资源。

### GQA 与 FlashAttention 的边界

GQA 让多个 query head 共享 K/V head，减少 KV cache 和读取成本，同时尽量保持 MHA 的表达能力 [25]。FlashAttention 用 IO-aware 的精确 attention kernel 减少显存读写 [24]。它们可以使更长上下文或更多候选变得可行，但不会改变模型是否正确推理。

算法实验报告应区分两种收益：质量收益来自更好的搜索、验证或训练目标；性能收益来自更少的 forward、更小的 cache 或更高效的 kernel。否则容易把“跑得更快”误解成“想得更好”，也无法比较同一算法在不同硬件上的可迁移性。
...
| 长上下文成本高 | 压缩、检索、GQA、cache 策略 | 只扩大 context window |
...
这套状态治理同样是推理算法能够长期演进的基础。

它让算法失败可以重现，让系统风险可以审计，也让成本优化有据可依。

也是后续迭代不可缺少的实验基础。

只有在状态、预算、验证和结果都被记录时，推理优化才不是凭感觉调参。

这也是可靠验收的必要条件。

它把算法研究和生产质量连接起来。

推理算法的效果必须在真实任务、真实预算和真实失败路径上反复验证。实验室中的平均分，只能说明方法值得继续研究，不能直接说明服务已经可靠。
...
生成算法从 greedy 和 sampling 发展到 CoT、Self-Consistency、显式搜索、验证器和 test-time scaling，核心变化是从“选择一个最可能的下一个 token”转向“在预算内寻找并验证一条可接受的任务轨迹”。Speculative decoding、GQA、FlashAttention 和 KV cache 策略解决的是执行效率，它们与推理质量相关但不等价。
