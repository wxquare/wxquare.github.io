...
把训练作业看成一个后端服务，会更容易做边界设计。控制面负责提交配置、分配 GPU、生成 rank 拓扑、登记数据版本、启动作业、保存状态和处理重试；数据面负责 GPU 上的 forward、backward、optimizer step，以及数据从对象存储进入 CPU 内存、Pinned Memory 和 GPU 显存的连续流动。两者不应混在一个脚本里：控制面需要幂等，数据面需要低延迟和高吞吐。用户重复提交同一个 job_id 时，平台应返回已有作业而不是再启动一个训练集群；worker 重启时，应根据 checkpoint 的全局步数和数据游标恢复，而不是依赖进程本地变量。

对后端工程师而言，训练配置应当是不可变的版本化对象。它至少包含模型 commit、tokenizer、数据 manifest、采样权重、并行拓扑、精度、随机种子、优化器超参数、容器镜像和依赖锁定文件。运行中的 job 只能产生状态事件，不能悄悄修改这些字段。若需要改变学习率或数据混合比例，应创建新的实验版本，并在元数据中记录父作业。这样做会增加少量控制面工作，却能避免“同名实验实际不可复现”的问题。Ray 将分布式任务和资源管理分离，为此类控制面提供了可借鉴的抽象 [26]。

### 训练通信的诊断路径

训练变慢时，不应立即增加 GPU。应先把一个 step 拆成数据等待、forward、backward、梯度同步、optimizer update 和 checkpoint 六个区间，再观察每个区间的 P50、P95 和最大值。数据等待占比高，优先检查 shard 数量、对象存储请求、解压线程、Pinned Memory 和 NUMA 绑定；梯度同步占比高，检查 tensor parallel 的消息大小、NCCL ring、跨节点带宽和进程映射；checkpoint 占比高，检查写放大、元数据操作和是否把保存动作放在了所有 rank 的同步路径上。

通信诊断必须结合拓扑。单机 NVLink、同机 PCIe、同 rack RoCE 和跨 rack 网络的带宽与延迟差异很大。一个在 8 卡机器上表现良好的 tensor parallel 度数，扩大到多个节点后可能被 all-reduce 延迟吞掉。NCCL 提供了集合通信和拓扑感知的基础能力，但应用仍需明确哪些通信是关键路径，并记录算法、协议和 peer 映射 [19]。工程上可先用小规模 microbenchmark 测量 all-reduce、all-gather、reduce-scatter，再用真实 batch 验证测量结果是否能解释 step 时间，避免被单一带宽数字误导。

### 故障域与弹性训练

GPU 训练的故障不是只有“进程退出”。常见故障包括单卡 Xid、节点重启、网络抖动、文件系统短暂不可写、对象存储限流、容器被驱逐、OOM、NaN 和某个 rank 卡死。它们的恢复策略不同：单卡故障可能需要替换节点并重建 communicator；文件系统故障可以退避重试；NaN 需要保存诊断快照并停止继续覆盖正常 checkpoint；rank 卡死则需要 watchdog 和全局超时，不能让其他 rank 永远等待集合通信。

弹性训练的核心不是“自动重启”，而是定义可接受的状态丢失边界。训练状态包括权重、master weights、梯度、optimizer moments、学习率调度器、随机数状态、数据游标和并行布局。只恢复权重而不恢复 optimizer，等价于改变了优化过程；只恢复 global step 而不恢复数据游标，可能重复或跳过样本。ZeRO、ZeRO-Infinity 和 FSDP 的分片策略改变了状态的物理分布，因此 checkpoint 格式需要保存 shard 元数据和逻辑参数名，而不能只依赖文件名排序 [2][3][18]。

### 训练成本的可解释归因

训练成本不能只按“GPU 小时”归因。更可用的单位是每百万有效 token 的成本，并同时展示失败重试、评估、checkpoint 和空闲等待的比例。若某次作业使用了 1,024 张卡但扩展效率只有 35%，它的名义 GPU 小时会掩盖真实浪费。平台应记录 reservation、实际占用、有效计算、通信等待和恢复重算五类时间，按团队、项目、模型版本和数据版本分摊。

成本归因还要区分探索成本与生产成本。研究实验可能允许较低利用率换取更快迭代；生产训练则更重视有效 token/s、可恢复性和可预测完成时间。二者不应共用同一条“利用率排行榜”，否则团队会为了指标牺牲实验质量，或为了实验便利消耗生产容量。Google Borg 的资源管理经验表明，配额、优先级、抢占和隔离需要作为平台策略统一处理，而不能由每个作业脚本自行决定 [21]。

### Checkpoint 的两阶段提交

可靠 checkpoint 可以采用“临时目录写入、完成标记提交”的两阶段协议。第一阶段由各 rank 将自己的 shard、优化器状态和元数据写入带有 job_id、step 和版本号的临时路径，并计算大小与校验和；第二阶段由协调者确认所有 shard 到齐、校验通过、全局元数据可读，再写入唯一的 COMMITTED 标记。恢复程序只接受存在 COMMITTED 标记的版本。这样可避免训练进程在写到一半时被杀死，导致下一次恢复误读半个 checkpoint。

完成标记还应包含逻辑 schema、world size、parallel degrees、dtype、tokenizer hash 和数据游标。若恢复时硬件数量不同，平台需要执行 reshard；若模型代码发生不兼容变化，应明确拒绝恢复而不是静默丢字段。checkpoint 目录中的每个文件都应有校验和，但校验和只证明传输完整，不证明状态语义正确，因此还应在恢复后做一次小 batch forward、loss 检查和参数统计。

### 数据迭代的可复现性

大规模数据集通常被切成很多 shard，并按 epoch、rank、worker 和 seed 共同决定读取顺序。仅记录一个随机种子是不够的：数据集版本变化、shard 数变化、worker 数变化都可能改变实际顺序。更稳妥的方式是把 dataset manifest、shard 列表、采样权重和当前逻辑游标写入 checkpoint，必要时保存已经消费的 shard 与样本范围。这样恢复时即使数据服务重新分片，也能明确选择“严格续跑”或“允许近似续跑”。

严格续跑有更强复现性，但会保存更多元数据并限制弹性；近似续跑允许丢弃当前 batch、重新打乱剩余 shard，恢复更快但会造成少量样本重复。两种模式都可以接受，关键是由作业配置显式选择，并在最终报告中披露。对于预训练，微小的数据顺序差异可能不影响最终指标；对于对齐和评估，样本重复可能造成偏差，不能用同一恢复策略一概而论。

### 性能优化的证据链

每次优化都应保留基线、假设、变更、测量和回滚条件。比如将 attention 替换为 FlashAttention，假设是减少 HBM 与片上存储之间的 IO；测量不仅包括 kernel 时间，还包括端到端 step、显存峰值、数值误差和不同序列长度下的退化。FlashAttention 的关键贡献正是把 IO 复杂度放进注意力实现的性能模型，而不是只比较 FLOPs [9][10]。如果优化只在固定长度 benchmark 上有效，却让不规则长度和小 batch 变慢，就不应直接推广。

后端工程中常见的“优化前后接口相同”在 GPU 系统里并不充分。优化可能改变显存分配峰值、通信顺序、算子精度和异常传播路径。验收需要加入 OOM、NaN、取消请求、超时和节点重启测试，并把结果写入模型运行时的版本记录。只有性能收益、质量不回退、故障行为可解释时，才把优化标记为默认路径。

### 容量模型：从 token 到 GPU

在线容量规划应从请求分布而非平均 QPS 开始。设到达率为 lambda，请求的输入 token 为 P、输出 token 为 D，单卡 decode 速率为 R_d，prefill 吞吐为 R_p，则粗略的计算需求可写成：GPU_demand 约等于 max(lambda×D/R_d，lambda×P/R_p) 除以目标利用率。这只是第一步，真实系统还受 KV cache 容量、batch 上限、网络传输、排队和尾延迟约束。若输出长度长尾明显，按平均 D 规划会导致高峰期排队；若上下文长度增长，prefill 计算和 KV 内存都会增长。容量模型应至少按短请求、中请求、长请求三类 workload 做压测，并记录每类请求的 goodput，而不是只看 tokens/s。

容量计划还要留出故障余量和发布余量。若集群在正常峰值时已经使用 90% GPU，任意一台节点故障都会把请求推入过载；灰度发布还需要额外副本，模型热加载需要临时双份权重。SRE 的错误预算思想可以延伸到容量预算：把可用容量的一部分保留给故障和变更，而不是把它当作可售卖容量 [22]。

### 请求生命周期与取消语义

推理请求应有明确的生命周期：accepted、queued、prefilling、decoding、streaming、completed、cancelled、failed。客户端断开连接后，服务端需要决定是立即取消 decode、继续生成并缓存结果，还是交给异步任务完成；不同产品不能靠框架默认行为。若不取消，用户已经离开的长请求仍会占据 KV cache 和 decode batch；若粗暴取消，又可能浪费已完成的前缀计算。

取消必须穿透网关、调度器、engine 和 GPU stream。状态更新要幂等，重复 cancel 不应把已完成请求改成失败。工具调用和多轮 agent 请求还需要区分“当前模型调用取消”和“整个工作流取消”。这些语义决定资源何时释放、计费如何计算以及 trace 是否完整，不能只在 HTTP 层增加一个 disconnect handler。

### 长上下文的内存账本

KV cache 的大小与层数、KV heads、head dimension、序列长度、batch 和 dtype 成正比。使用 GQA 或 MQA 能减少 KV heads，但不会消除所有开销；prefix cache 可以复用共享前缀，却要付出哈希、租约、失效和安全隔离成本。平台应在请求进入调度器前估计 cache 需求，并为每个租户设置最大上下文、最大并发和最大保留时间。

长上下文还会改变公平性。一个持续生成的请求不断占用 decode slot，短请求可能排队；一个超长 prefill 又可能让所有 decode 停顿。Sarathi-Serve 的 chunked prefill 说明，把长 prefill 切成可调度块能够更好地平衡吞吐和延迟 [14]。工程实现中需要给每个 chunk 设置预算、优先级和可抢占点，并在指标中区分等待 prefill、等待 decode 和等待 cache 的时间。

### 负载隔离与容量预留

在线推理常常同时承载交互式问答、批量离线任务、评估回归和 agent 工作流。它们的延迟目标不同，若共享一个无优先级的队列，离线任务会挤压交互请求，或交互请求的突发使批处理反复被打断。调度器应区分租户、业务、请求类型和优先级，并定义抢占规则。高优先级不意味着无限资源，而是拥有明确的并发上限和可审计的配额。

隔离要覆盖 GPU、CPU、内存、网络和缓存。仅设置 Kubernetes 的 GPU limit，不能阻止某个租户占满 CPU 解码、对象存储连接或 prefix cache。Kubernetes 的设备插件和调度机制提供了 GPU 分配基础，但具体的拓扑、MIG、节点亲和性和驱逐策略仍需平台层实现 [20]。对共享模型权重，可以只读挂载；对租户专属 adapter 和 cache，则要使用独立命名空间或加密 key，防止跨租户命中造成数据泄露。

### 发布、灰度与回滚的兼容性

回滚必须测试过而不是写在文档里。至少要验证权重、tokenizer、chat template、量化引擎、adapter、路由规则和安全策略能回到上一组兼容版本。若新版本已经产生了新的 prefix cache，回滚时应丢弃不兼容 cache；若模型输出已经进入下游系统，还要明确业务侧是否接受语义回滚。模型注册中心应保存每个 artifact 的 hash、来源、构建日志、评估结果和审批记录。

灰度指标不能只看 HTTP 5xx。还需要比较首 token 延迟、每输出 token 延迟、长度分布、停止原因、工具调用成功率、拒答率、离线评估分数和单位成本。新模型可能延迟更低但生成更长，或者错误率不变但高价值任务质量下降。发布控制器可以把这些指标按租户和场景分层，对明显回退自动暂停，对不确定的变化进入人工评审。这样，模型发布才接近后端服务的 canary，而不是简单替换容器镜像。

### 故障模型与演练

故障演练需要覆盖从单请求到整集群的不同范围。单请求层测试超时、取消、非法参数、流式连接断开和上游重试；实例层测试 worker OOM、engine 崩溃、模型加载失败和 cache 损坏；集群层测试节点丢失、网络分区、对象存储不可用和调度器故障。每项演练都应定义注入动作、预期信号、用户影响、自动恢复时间和人工介入边界。

训练和推理的故障演练重点不同。训练需要检查 checkpoint 是否能在新节点恢复、恢复后 loss 是否连续、数据游标是否正确，以及重算成本是否符合预算；推理需要检查流量是否被转移、KV cache 是否释放、排队是否受控，以及降级模型是否保持接口和安全策略。演练结果应进入版本发布门禁，未通过的系统不能以“平时没发生过”为理由上线。

### 面向后端工程师的接口契约

大模型平台的接口至少要定义四类契约。第一类是输入契约：模型名、版本、tokenizer、消息格式、最大 token、工具 schema 和采样参数；第二类是资源契约：超时、并发、优先级、预算和取消；第三类是结果契约：流式事件、finish reason、usage、引用、工具调用和错误码；第四类是治理契约：调用方身份、数据地域、审计字段、保留期和脱敏策略。

如果只定义一个“生成文本”接口，调用方会把重试、截断、工具状态和计费逻辑各自实现，最终产生不可控差异。建议使用 request_id、model_revision、trace_id 和 idempotency_key 贯穿网关、调度器、engine 与计费系统。对于流式响应，要定义事件顺序和断线恢复语义；对于异步批任务，要定义提交、查询、取消和过期状态。接口设计越明确，后续更换 vLLM、TensorRT-LLM 或自研 engine 的成本越低 [12][16]。

### 端到端验收矩阵

一套可执行的验收矩阵应把场景、指标和证据绑定起来。功能层验证 tokenizer 与模板、普通生成、长上下文、工具调用、流式和取消；性能层验证 TTFT、TPOT、吞吐、P95/P99 和显存峰值；可靠性层验证节点故障、重启、回滚、恢复和降级；治理层验证权限、审计、成本、数据保留和跨租户隔离。每项都要记录硬件、模型 hash、数据集、并发模型和测试时间，否则数字无法比较。

训练验收可以使用固定 token 数而不是固定 wall-clock，比较有效 token/s、MFU、通信占比、loss 曲线和 checkpoint 恢复后的连续性；推理验收可以使用固定请求混合和固定质量阈值，比较 goodput、成功请求成本和尾延迟。对于算法变更，必须同时报告质量和系统指标；对于 kernel 或调度变更，必须证明输出质量与数值稳定性没有超出容差。只有把这些证据保留在模型注册和发布记录中，平台才拥有可持续演进的基础。

### 端到端的状态边界

大模型系统最容易被忽略的问题是状态散落在不同组件中。网关保存请求状态，调度器保存排队状态，推理引擎保存 KV block，工具服务保存副作用状态，计费系统保存 token 账单，观测系统保存 trace。如果没有明确的状态所有者，重试就可能产生重复扣费、重复工具调用或幽灵请求。每种状态都应回答三个问题：谁创建，谁更新，谁在故障后负责清理。把状态写入数据库并不自动解决问题，仍需要版本、过期时间、幂等键和一致的状态迁移。

对于同步生成，request_id 可以作为全链路关联键，但不能直接作为副作用操作的幂等键，因为一次请求可能包含多个工具动作。工具调用应有独立的 operation_id，并把模型生成的参数、审批结果、执行结果和重试次数记录在状态机中。对于异步任务，应把提交和执行解耦：提交接口只负责持久化合法任务并返回 job_id，worker 负责领取、续租和上报心跳，查询接口读取状态而不是依赖 worker 内存。这样的设计既适用于批量推理，也适用于评估和训练。

### 数据平面背压

所有层都需要背压，否则局部流量峰值会沿链路放大。网关可以按 token 预算拒绝请求，调度器可以限制每个租户的 queued token，engine 可以根据 KV 水位暂停接纳长上下文，结果流可以限制客户端读取速度。背压要有明确错误语义，例如资源暂时不足返回可重试错误，超过配额返回不可重试错误，安全策略拒绝则不能被客户端无限重试。

重试策略必须考虑生成请求的非确定性和副作用。网络连接在首 token 之后断开时，客户端无法知道服务端是否已经完成工具调用；盲目重试可能重复发送邮件、创建订单或写入数据库。可靠做法是把模型调用和副作用提交拆开，使用幂等 key、状态查询和明确的 commit 阶段。对纯文本生成可以重试，对工具动作则优先查询原 operation_id。后端系统已有的幂等、租约和事务思想，在这里仍然成立，只是状态更长、资源更贵。

### 从实验环境到生产环境

实验环境关注“能否快速试验”，生产环境关注“能否长期解释”。两者可以共用运行时镜像和模型制品，但不应共用无边界的凭证、对象存储路径和在线流量。实验任务可以使用抢占式 GPU，但必须标记可中断；生产推理需要稳定节点、发布审批和容量余量。开发者可以查看抽样 prompt 帮助调试，但生产日志应默认脱敏并通过审计访问。

迁移到生产前，平台应完成一次从 manifest 到 artifact、从 artifact 到 engine、从 engine 到 canary 的完整演练。演练要包含冷启动、热扩容、版本回滚、cache 清理、节点故障和指标恢复。若某一步依赖人工登录机器拷贝文件，它就不是可重复的发布流程，而是隐藏的运维手册。把构建、签名、注册、部署和验收串成流水线，才能使模型迭代速度不以可靠性为代价。

### 本章小结：把复杂度变成契约

Infra 的核心能力不是堆叠组件，而是把算法复杂度转化为资源契约、状态契约和质量契约。资源契约说明一个模型在不同长度、并发和硬件下需要多少计算、显存、网络与存储；状态契约说明请求、KV、checkpoint、数据游标和工具操作如何创建、恢复、过期与回滚；质量契约说明模型升级、量化、kernel、调度和降级对输出质量与用户任务的影响。

后续章节将把本章的总览拆成五条更具体的工程链路：训练 Infra 关注数据供给、并行和 checkpoint；推理 Infra 关注 serving、KV 和 batching；数据与评估 Infra 关注数据治理和回归闭环；运行时 Infra 关注 agent、工具、工作流和租户；可靠性与治理 Infra 关注观测、成本、安全、SLO 和灾备。读者不必先记住所有组件名称，但应始终追问同一个问题：这个算法选择改变了哪项资源约束，平台用什么指标证明它在真实负载下仍然成立？

### 设计评审中的常见误区

第一个误区是用单一指标替代系统目标。GPU 利用率高，不代表有效 token/s 高；平均延迟低，不代表 p99 满足交互场景；模型成本低，不代表单位成功任务成本低。评审时应把指标放在因果链上：输入长度和输出长度如何改变 prefill 与 decode，调度策略如何改变队列，cache 命中如何改变计算，模型质量如何改变重试和人工接管。只有链路闭合，优化数字才有解释力。

第二个误区是把组件能力当成平台能力。某个 serving engine 支持 continuous batching，不等于平台已经解决租户配额、取消、审计和回滚；某个训练框架支持分片 checkpoint，不等于 checkpoint 已经具备跨版本恢复和数据游标；某个观测系统能采集 trace，不等于敏感 prompt 可以安全记录。组件提供机制，平台仍要定义契约、默认值和验收。

第三个误区是只测试正常路径。真实事故往往发生在长上下文、流式断开、模型发布、节点故障、存储抖动和流量突发的交界处。测试集应覆盖短输入与长输入、短输出与长输出、命中与未命中、单租户与多租户、冷启动与热运行、正常取消与重复取消。对训练，还要覆盖中途保存、恢复时改变 world size、部分 shard 丢失和数据版本不可读。这样得到的不是漂亮的 demo，而是可以承担生产负载的系统。

### 平台演进的最小闭环

一个可持续演进的平台可以从最小闭环开始：模型注册表保存不可变 artifact；训练控制面生成可复现 job；数据 manifest 定义输入；运行时暴露统一接口；指标和 trace 关联请求与资源；发布控制器执行灰度和回滚；成本系统按 token、GPU 和成功任务归因。这个闭环不要求第一天就拥有复杂的异构调度或跨区域灾备，但每个新增能力都必须接入版本、权限、观测和故障处理。

当模型规模扩大时，先增强测量再增强自动化。没有扩展曲线就无法决定是否引入新的并行维度，没有 KV 水位和请求分布就无法决定是否拆分 prefill/decode，没有失败与恢复记录就无法决定 checkpoint 频率。自动化的价值是重复执行已经定义清楚的决策，而不是替代指标和边界。对一个有后端经验的工程团队来说，这种渐进路线通常比一次性搭建“全功能 AI 平台”更容易验证，也更容易在业务变化时保留可迁移性。

### 从请求级指标到任务级指标

token 是 Infra 中最方便的统一单位，却不是最终业务结果。一个请求可能输出很多 token 但没有完成任务，也可能只输出很短的结构化结果却触发了昂贵的检索、工具和人工审核。平台应把请求级指标向任务级指标传递：一次 agent 任务由多少模型调用组成，调用之间等待了什么，工具是否成功，是否发生重试，最终是否需要人工接管。任务级 trace 可以把这些步骤串起来，再将总 token、GPU 时间、缓存收益和失败原因归因到同一个任务。

任务级指标也能帮助做容量决策。若某个工作流平均包含四次模型调用，第三次调用失败会导致前三次资源全部浪费，那么提高单次请求吞吐可能不如降低工具失败率更有价值；若一个检索任务因为上下文过长频繁触发截断，增加 GPU 并不能解决问题，需要在数据和上下文编排层缩短输入。Infra 不是脱离产品的底层管线，它最终要用成功任务、单位成本和用户等待时间来评价。

### 版本化的运行时契约

模型、tokenizer、模板、量化、kernel、调度器和安全策略应组成一份运行时契约。契约可以用机器可读的 manifest 表达：artifact hash、兼容的硬件、支持的最大上下文、精度、停止条件、工具协议、默认超时和质量基线。部署控制器在加载前校验契约，发现 tokenizer 与模板不匹配、engine 与 GPU 架构不兼容或质量评估过期时直接拒绝发布。

契约版本还要支持兼容性判断。新增可选字段通常可以向后兼容，改变 tokenization、停止语义、结构化输出 schema 或工具副作用则需要主版本升级。网关、SDK、评估系统和计费系统都应读取同一个版本，而不是各自猜测模型行为。这样即使底层从一种 serving engine 迁移到另一种实现，外部系统仍然依赖稳定的能力边界，内部才有空间持续优化。

最终，平台的成熟度体现在变化发生时仍能回答清楚：变更影响了哪些请求，消耗了多少资源，质量是否退化，谁批准了发布，出现问题如何停止和恢复。模型规模、硬件类型和运行时都会变化，但版本化制品、可观测指标、幂等状态、容量预算和可执行回滚这些原则不会变化。它们是把一次性的模型实验转化为长期可维护服务的最小共同基础。

在实际评审中还应把“默认行为”写出来。默认最大输入和输出长度、默认超时、默认重试次数、默认 cache 保留时间、默认租户并发、默认降级模型和默认日志脱敏规则，都会在流量放大后变成平台事实。未声明的默认值通常由某个库、某个容器环境变量或某位同事的脚本决定，最终难以审计。更可靠的做法是把默认值写入版本化配置，启动时打印配置摘要，在请求 trace 中记录真正生效的值，并为每项配置设置变更审批和回滚版本。这样能够把隐含行为变成可见的工程约束，也能让不同环境的压测结果具有可比性。

这些约束还需要和算法侧的基础假设对齐。DeepSpeed 的系统优化、FSDP 的分片接口和 Llama 3 的训练报告提醒我们，模型规模、数据、硬件和运行时不能被当作彼此独立的变量 [4][6][8]。中文教材对优化、泛化和数据分布的讨论也说明，平台验收不能只测吞吐，还要检查训练曲线、评估稳定性与数据处理语义 [28][29][30]。Infra 团队至少要和算法团队共同维护一份假设清单，明确哪些变化属于兼容优化，哪些变化会改变实验含义。

这份假设清单应进入发布门禁：模型参数或上下文变化时重新计算显存，数据配方变化时重新检查吞吐和评估，通信拓扑变化时重新测扩展效率，运行时升级时重新做恢复和回滚。用统一清单连接算法和平台，能够减少“代码能启动但实验不可比”的隐性风险，也为后续章节的训练、推理和治理拆分提供共同语言。

这也使平台的每一次优化都有明确的影响范围、验证方法和回滚条件。
