...
这一部分讨论如何把大模型组装为真正可运行的 Agent。它从 Agent 架构总纲出发，依次展开任务协议、上下文、Harness、模型协议、工具、知识、记忆、编排和生产治理。
...
这一部分将前两部分的能力放到现实工作中：先拆解成熟产品，再落到企业、个人和研究型任务。它关心的是“如何用大模型与 Agent 完成一项可交付、可验证、可治理的真实工作”。
...
- 普通问答、RAG 问答与 Research Agent 的边界，以及研究型 Agent 的现实瓶颈。
...
6. 如果你关注 AI 搜索、研究助手或内容生产，再读第 26 章。
...
8. 需要做 Deep Research、研究型 Agent 或研究系统综述时读第 26 章；
9. 准备面试或作品集时查阅附录D。
