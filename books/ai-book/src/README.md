...
本书从大模型基础出发，逐层进入 Agent Runtime、知识系统、生产治理、成熟系统解析、实战案例，以及 AI Research 工作流与知识生产。主线不是“学一堆工具名”，而是把不稳定的模型能力放进可复用、可审查、可验证、可治理的工程系统。
...
这一部分把前两部分的方法放到真实系统中观察，分成两段：第 17-20 章是成熟系统解析，第 21-24 章是实战案例。
...
这一部分把前面的方法进一步收束到研究型工作流上，重点讨论 Research Agent 的定义边界、发展脉络、评测现状、关键研究方向，以及它与理想研究系统之间的差距。

你会获得：

- 普通问答、RAG 问答与 Research Agent 的边界；
- Research Agent 的历史演进、当前系统栈与代表性基准；
- 长程研究任务中的核心瓶颈，如记忆治理、安全边界与评测碎片化；
- 理想研究型智能体应具备的能力，以及当前前沿系统的现实差距；
- 面向研究、企业与治理场景的落地建议。
...
6. 如果你关注 AI 搜索、研究助手或内容生产，再读第 25 章。
...
4. 阅读第四部分，理解 AI Research 如何落到证据驱动工作流和知识资产沉淀；
...
2. 需要接外部系统时读第 6 章；
3. 需要处理知识库或私有数据时读第 7 章；
4. 需要设计 Memory 和长期上下文时读第 8 章；
5. 需要编排多步任务、状态机、多 Agent 或平台框架选型时读第 9 章；
6. 准备上线前读第 10 章；
7. 想参考成熟系统时读第 17-24 章；
8. 需要做 Deep Research、研究型 Agent 或研究系统综述时读第 25 章；
9. 准备面试或作品集时查阅附录D。
