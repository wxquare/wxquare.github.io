...
商品供给与运营治理平台解决的是“商品如何进入平台、如何被审核发布、上线后如何持续维护”的问题。它不是运营后台的 CRUD，也不是供应商同步的一组定时任务，而是一条长期运行的供给治理流水线。

在数字商品平台中，商品供给来源通常有四类：
...
供应商同步属于商品供给链路，但它不是商品供给链路的全部。更合理的设计是：用一套统一的供给治理控制面承接四类入口，共享任务模型、暂存区、校验、审核、发布版本、Outbox、DLQ、补偿和质量监控；其中供应商同步因为有长任务、Checkpoint、Raw Snapshot、Worker 租约和数据新鲜度问题，单独作为专项链路展开。供应商同步的完整设计见[附录F：供应商数据同步链路](./supplier-sync.md)。

本附录聚焦统一供给治理平台，尤其补足人工上传、批量导入和运营编辑三条链路。

## 2. 设计目标

1. **入口统一**：人工创建、批量导入、运营编辑、供应商同步进入统一任务和发布框架。
2. **线上隔离**：所有未校验、未审核、未发布的数据只进入 Draft / Staging，不污染正式商品表。
3. **质量可控**：通过类目模板、主数据校验、交易契约校验、风险规则和审核流控制发布质量。
4. **发布一致**：商品主数据、资源映射、Offer、库存配置、履约规则、退款规则、搜索索引和缓存最终一致。
...
| 难点 | 典型表现 | 风险 | 解决方法 |
|------|----------|------|----------|
| 入口多且语义不同 | 人工创建、批量导入、运营编辑、供应商同步都在改商品 | 流程混乱、重复逻辑、审计缺失 | 统一为 Supply Task，但按 `task_type` 路由不同策略 |
...
| 发布不一致 | DB 成功，ES / 缓存 / 营销 / 计价没刷新 | 搜不到、价格错、下单失败 | 发布事务 + Outbox + 异步刷新 + 补偿重试 |
...
Supply Entry
  ...
  → Publish Transaction
  → Outbox Event
  → Search / Cache / Pricing / Marketing / Data Platform
...
| 集成层 | 通知搜索、缓存、营销、计价、数据平台 | Outbox、索引任务、缓存失效任务 |
...
商品供给与运营平台不应该替代商品中心、库存系统或计价系统。它的职责是“供给流程和发布治理”，不是所有商品数据的唯一存储。

| 系统 | 负责什么 | 不负责什么 |
|------|----------|------------|
| 供给与运营平台 | 入口、任务、暂存、校验、审核、发布编排、补偿、审计 | C 端高 QPS 商品查询 |
| 商品中心 | Resource、SPU、SKU、Offer、Rate Plan、类目、属性正式模型 | 运营任务进度和错误文件 |
| 库存系统 | 库存来源、库存扣减、券码池、实时可售 | 商品标题、图片、类目 |
...
2. 库存、价格、履约、退款规则在发布时形成交易前契约。
3. 下游通过 Outbox 事件感知变更，不由运营后台直接调用所有系统。
4. 订单只相信创单时保存的快照，不回读最新商品配置解释历史订单。
...
ES、缓存、营销、计价不要放在发布事务里同步调用，统一由 Outbox 消费者异步刷新。
...
> 我不会把商品供给和运营链路设计成后台 CRUD。我的设计是统一供给治理平台：人工创建、批量导入、运营编辑和供应商同步都先进入任务和暂存区，经过标准化、质量校验、Diff、风险审核、版本化发布、Outbox、补偿和质量巡检后，才写入正式商品主数据和交易契约。系统难点不在于建几张商品表，而在于入口语义统一、线上隔离、类目差异、批量部分成功、运营误操作防护、字段主导权、发布一致性、订单快照和失败运营化。供应商同步属于供给链路，但因为它有 Checkpoint、Raw Snapshot、Worker 租约和数据新鲜度问题，所以作为专项链路单独设计。
...
参考回答：是。供应商同步是商品供给的一种自动化入口，但不是全部。统一供给平台要承接人工创建、批量导入、运营编辑和供应商同步。供应商同步因为有长任务、Checkpoint、Raw Snapshot、Worker 租约和新鲜度问题，所以需要专项链路。
...
参考回答：商品正式表更新和 Outbox 事件写入同一个事务。事务提交后，由异步消费者刷新 ES、缓存、营销、计价和数据平台。刷新失败进入补偿任务，保证最终一致。
