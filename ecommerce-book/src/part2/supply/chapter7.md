...
参考回答：商品供给平台负责 Draft、Staging、QC、批量导入、供应商同步、DLQ 和发布编排；商品中心负责正式商品主数据、交易前契约、发布版本、商品快照和读模型。供给平台是“商品如何进入平台”，商品中心是“正式商品如何被交易系统稳定使用”。
...
商品中心不是供给后台，也不是简单 SPU/SKU CRUD。一个成熟的商品中心应该是正式商品主数据和交易前契约中心：用 Resource、Product Item、SPU/SKU、Offer / Rate Plan 表达“卖什么”和“怎么卖”，用库存配置、Input Schema、履约规则、退款规则表达“如何下单、如何履约、如何售后”，用 `publish_version` 和商品快照保证历史订单可解释，用 Outbox 保证搜索、缓存、营销、计价等下游最终一致。
